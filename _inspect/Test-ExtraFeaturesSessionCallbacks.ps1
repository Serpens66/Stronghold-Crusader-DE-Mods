...
$lord=[IO.File]::ReadAllText((Join-Path $root 'ExtraFeatures/src/LordHealthRuntime.cs'))
$policy=[IO.File]::ReadAllText((Join-Path $root 'ExtraFeatures/src/LordHealthMultiplierPolicy.cs'))
$market=[IO.File]::ReadAllText((Join-Path $root 'ExtraFeatures/src/AIMarketVanillaPriceHook.cs'))
$runtime=[IO.File]::ReadAllText((Join-Path $root 'ExtraFeatures/src/ExtraFeaturesRuntime.cs'))
$oldMarket=(& git -C $root show '7e048e6435a72b339dff35f3dcd368210f04ccdb:ExtraFeatures/src/AIMarketVanillaPriceHook.cs') -join "`r`n"
foreach($method in @('private int GetBuyPrice(', 'private int GetSellPrice(')) {
    if((Block $market $method) -cne (Block $oldMarket $method)){throw "Price arithmetic/trampoline/breadcrumb callback changed: $method"}
}
if(([regex]::Matches($runtime,'lordHealthRuntime\.BeginMap\(')).Count -ne 1 -or
   -not (Block $runtime 'private void OnSessionStarted(').Contains('lordHealthRuntime.BeginMap(context.SessionId, context.IsLoadedSave)')) {throw 'Lord settings are not captured exclusively at session start'}
foreach($method in @('private void OnUnloadMap(', 'private void OnModeStateChanged(')) {
    $body=Block $runtime $method
    if(-not $body.Contains('SetSessionOverride(false)') -or -not $body.Contains('lordHealthRuntime.ResetMapState()')){throw "Missing retirement: $method"}
}
$code=@'
using System;
...
namespace ExtraFeatures {
    ...
    public class ExtraFeaturesViewModel { public int HumanLordHealthPercent=200,AILordHealthPercent=200; public bool EnableMod=true,MarketPricesAlsoForAI; }
'@
$code+=(Block $lord 'internal sealed unsafe class LordHealthRuntime')+"`n"+(Block $policy 'internal static class LordHealthMultiplierPolicy')
$code+="`npublic class MarketHarness { private volatile bool useVanillaAIPricesForSession;`n"+(Block $market 'internal void SetSessionOverride(')+"`n"+(Block $market 'private bool ShouldUseVanillaPrice(').Replace('private bool','public bool')+"`n}`n"
$code+="public class SessionHarness { public MarketHarness aiMarketVanillaPriceHook=new MarketHarness(); public bool mapActive; public ExtraFeaturesViewModel settings=new ExtraFeaturesViewModel();`n"+(Block $runtime 'private void CaptureAIMarketSessionSettings(').Replace('private void','public void')+"`n}`n"
$code+=@'
...
'@
$code+="`r`npublic static class Program { public static void Main() { System.Console.WriteLine(ExtraFeatures.SessionTests.Run()); } }`r`n"
$harness=Join-Path $PSScriptRoot 'ExtraFeaturesSessionTests/GeneratedHarness.cs'
$code=[regex]::Replace($code,'\r?\n',"`r`n")
[IO.File]::WriteAllText($harness,$code,[Text.UTF8Encoding]::new($false))
if([IO.File]::ReadAllText($harness) -cne $code){throw 'Generated harness write mismatch'}
dotnet msbuild (Join-Path $PSScriptRoot 'ExtraFeaturesSessionTests/ExtraFeaturesSessionTests.csproj') /t:Build /p:Configuration=Release /verbosity:minimal
if($LASTEXITCODE -ne 0){throw 'Session test compilation failed'}
& (Join-Path $PSScriptRoot 'ExtraFeaturesSessionTests/bin/ExtraFeaturesSessionTests.exe')
