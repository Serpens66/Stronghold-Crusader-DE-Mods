...
public class HUD_IngameMenu : UserControl
{
	...
	public class RestartSkirmishMapInfo
	{
		...
		public static RestartSkirmishMapInfo decode(byte[] data, bool customTrailMission = false)
		{
			try
			{
				...
				if (num2 >= 53)
				{
					int num7 = BitConverter.ToChar(data, num);
					num++;
					...
					for (int j = 0; j < restartSkirmishMapInfo.aivs.Length; j++)
					{
						...
						if (num2 >= 58)
						{
							...
							try
							{
								...
								if ((BaseComponent)(object)restartSkirmishMapInfo.aivs[j].image != (BaseComponent)null && (((ImageSource)restartSkirmishMapInfo.aivs[j].image).Width != 144f || ((ImageSource)restartSkirmishMapInfo.aivs[j].image).Height != 144f))
								{
	...
	public class RestartMPInfo
	{
		...
		public TextureSource[] images = (TextureSource[])(object)new TextureSource[8];
		...
		public void SetImage(byte[] data, int i)
		{
			...
			try
			{
				...
				if ((BaseComponent)(object)images[i] != (BaseComponent)null && (((ImageSource)images[i]).Width != 144f || ((ImageSource)images[i]).Height != 144f))
				{
					images[i] = null;
					imageData[i] = null;
				}
				...
				for (int i = 0; i < 8; i++)
				{
					...
					if (num4 > 0)
					{
						...
						try
						{
							...
							if ((BaseComponent)(object)restartMPInfo.images[i] != (BaseComponent)null && (((ImageSource)restartMPInfo.images[i]).Width != 144f || ((ImageSource)restartMPInfo.images[i]).Height != 144f))
							{
	...
	public Button RefButtonLoad;

	public Button RefButtonWorkshop;

	public Button RefButtonSave;

	public Button RefButtonOptions;

	public Button RefButtonHelp;

	public Button RefButtonRestart;

	public Button RefButtonQuit;

	public Button RefButtonExit;

	public Button RefButtonResume;
	...
	public HUD_IngameMenu()
	{
		...
		RefButtonLoad = (Button)((FrameworkElement)this).FindName("ButtonLoad");
		RefButtonWorkshop = (Button)((FrameworkElement)this).FindName("ButtonWorkshop");
		RefButtonSave = (Button)((FrameworkElement)this).FindName("ButtonSave");
		RefButtonOptions = (Button)((FrameworkElement)this).FindName("ButtonOptions");
		RefButtonHelp = (Button)((FrameworkElement)this).FindName("ButtonHelp");
		RefButtonRestart = (Button)((FrameworkElement)this).FindName("ButtonRestartMission");
		RefButtonQuit = (Button)((FrameworkElement)this).FindName("ButtonQuitMission");
		RefButtonExit = (Button)((FrameworkElement)this).FindName("ButtonExitStronghold");
		RefButtonResume = (Button)((FrameworkElement)this).FindName("ButtonResume");
	}

	public void InitializeComponent()
	{
		NoesisUnity.LoadComponent((object)this, "Assets/GUI/XAMLResources/HUD_IngameMenu.xaml");
	}
	...
	public void Init()
	{
		...
		((FrameworkElement)RefLayoutRoot).Height = 400f;
	...
	public void SetAsNormal()
	{
		...
		((UIElement)RefButtonLoad).IsEnabled = true;
		((UIElement)RefButtonSave).IsEnabled = Director.instance.SimRunning;
		((UIElement)RefButtonOptions).IsEnabled = true;
		((UIElement)RefButtonHelp).IsEnabled = true;
		if (GameData.Instance.game_type == 0)
		{
			((UIElement)RefButtonRestart).IsEnabled = true;
		}
		else if (GameData.Instance.game_type == 2)
		{
			((UIElement)RefButtonRestart).IsEnabled = restartMapInfo != null;
		}
		else if (Director.instance.SkirmishModeGame && GameData.Instance.SkirmishGameType != 1)
		{
			((UIElement)RefButtonRestart).IsEnabled = restartSkirmishMapInfo != null;
		}
		else
		{
			((UIElement)RefButtonRestart).IsEnabled = true;
		}
		((UIElement)RefButtonQuit).IsEnabled = true;
		((UIElement)RefButtonExit).IsEnabled = true;
		((UIElement)RefButtonResume).IsEnabled = true;
		((UIElement)RefButtonLoad).Visibility = (Visibility)2;
		((UIElement)RefButtonWorkshop).Visibility = (Visibility)0;
		((UIElement)RefButtonRestart).Visibility = (Visibility)2;
	...
	public void SetAsTutorial()
	{
		...
		MainViewModel.Instance.IngameMessageRestartButtonText = Translate.Instance.lookUpText(Enums.eTextSections.TEXT_GAME_OPTIONS, 44);
		MainViewModel.Instance.IngameMessageQuitButtonText = Translate.Instance.lookUpText(Enums.eTextSections.TEXT_GAME_OPTIONS, 7);
		((UIElement)RefButtonLoad).IsEnabled = false;
		((UIElement)RefButtonSave).IsEnabled = false;
		((UIElement)RefButtonOptions).IsEnabled = true;
		((UIElement)RefButtonHelp).IsEnabled = true;
		((UIElement)RefButtonRestart).IsEnabled = false;
		((UIElement)RefButtonQuit).IsEnabled = true;
		((UIElement)RefButtonExit).IsEnabled = true;
		((UIElement)RefButtonResume).IsEnabled = true;
		((UIElement)RefButtonLoad).Visibility = (Visibility)2;
	...
	public void SetAsMapEditor()
	{
		...
		((UIElement)RefButtonExit).IsEnabled = true;
		((UIElement)RefButtonResume).IsEnabled = true;
		((UIElement)RefButtonRestart).Visibility = (Visibility)0;
		((UIElement)RefButtonWorkshop).Visibility = (Visibility)2;
		if (GameData.Instance.multiplayerMap)
		{
			...
			{
				((UIElement)RefButtonWorkshop).IsEnabled = true;
			}
			else
			{
				((UIElement)RefButtonWorkshop).IsEnabled = false;
			}
		}
		else
		{
	...
	public void SetAsMultiplayer()
	{
		...
		MainViewModel.Instance.IngameMessageQuitButtonText = Translate.Instance.lookUpText(Enums.eTextSections.TEXT_GAME_OPTIONS, 7);
		((UIElement)RefButtonLoad).IsEnabled = false;
		((UIElement)RefButtonSave).IsEnabled = Director.instance.SimRunning && Platform_Multiplayer.Instance.IsHost;
		((UIElement)RefButtonOptions).IsEnabled = true;
		((UIElement)RefButtonHelp).IsEnabled = true;
		((UIElement)RefButtonRestart).IsEnabled = false;
		((UIElement)RefButtonQuit).IsEnabled = true;
	...
	public static void SaveMapEditor(bool fromIngameMenu)
	{
		Action<string, FileHeader> oKAction = delegate(string filename, FileHeader header)
		{
			...
			string path = Path.Combine(ConfigSettings.GetUserMapsPath(), text);
