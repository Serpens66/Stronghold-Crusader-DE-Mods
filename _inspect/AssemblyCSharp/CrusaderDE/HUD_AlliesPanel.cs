...
public class HUD_AlliesPanel : UserControl
{
	...
	private const int ORDER_NONE = 0;

	private const int ORDER_ATTACK = 1;

	private const int ORDER_DEFEND = 2;

	private int selectedGoods = -1;

	private int selectedGoodsAmount = -1;

	private int req_goods_asked_for;

	private DateTime nextAllyCheck = DateTime.MinValue;

	private int curr_ally;

	private int num_allies;

	private int[] ally = new int[6];

	private int numEnemies;

	private int[] sk_enemy = new int[6];

	private bool lastAlliesPip;

	private DateTime lastAlliesPipTime = DateTime.MinValue;
		...
		for (int i = 0; i < num_allies; i++)
		{
			...
			if (lastGameState.is_skirmish_player(num))
			{
				if (curr_ally == i)
				{
					...
					UnityEngine.Color color = GameMap.Lighten(OnScreenText.Instance.MPTeamColours[SpriteMapping.remapColours[SpriteMapping.RemapMPLoadedColour(num)]], 0.3f);
					MainViewModel.Instance.AlliesSelectedLordColour = new SolidColorBrush(Noesis.Color.FromArgb(byte.MaxValue, (byte)(color.r * 255f), (byte)(color.g * 255f), (byte)(color.b * 255f)));
				}
				else
				{
	...
	{
		Noesis.GUI.LoadComponent(this, "Assets/GUI/XAMLResources/HUD_AlliesPanel.xaml");
	}
