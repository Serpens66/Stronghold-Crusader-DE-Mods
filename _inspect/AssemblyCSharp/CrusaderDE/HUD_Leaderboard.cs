...
public class HUD_Leaderboard : UserControl
{
	...
	private RadioButton refFriendsButton;

	private RadioButton refGlobalButton;

	public bool pageTypeFriends = true;

	public int currentPage = -1;

	private bool retryGlobal;

	private bool pending;

	private bool pending_Friends;

	private int pending_Page;
	...
	public static void OpenLeaderboard(string leaderboardName, string leaderboardTitle)
	{
		...
		Instance = (HUD_Leaderboard)FatControler.instance.FindVisibleUIElement(typeof(HUD_Leaderboard));
		Instance.Init();
	...
	private void Init()
	{
		refFriendsButton = (RadioButton)FindName("FriendsButton");
		refGlobalButton = (RadioButton)FindName("GlobalButton");
		refFriendsButton.IsChecked = true;
	...
	private bool UpdateLeaderboard(string leaderboardName, bool friendsOnly, int page, bool force = true)
	{
	...
	private void SetLeaderboardRow(int row, string friendsPosition, string position, string name, string time, ImageSource avatar)
	{
		switch (row)
		{
		case 0:
	...
	private void SetLeaderboardRowAvatar(int row, ImageSource avatar)
	{
		switch (row)
		{
		case 0:
	...
	{
		GUI.LoadComponent(this, "Assets/GUI/XAMLResources/HUD_Leaderboard.xaml");
	}
}
