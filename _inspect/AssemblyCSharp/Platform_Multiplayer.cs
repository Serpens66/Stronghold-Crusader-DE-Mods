...
public class Platform_Multiplayer
{
	...
	public class MPLobbyMember
	{
		...
		public string Name
		{
			get
			{
				...
				return GetName(lord, lordSubType, customLordName);
			}
			set
			{
				name = value;
		...
		public static string GetName(int lord, int subType, string name = "")
		{
			switch (lord)
			{
			case 0:
			...
			case 36:
			case 37:
				if (CustomisationFileManager.CustomMediaExists && name != null && name != "")
				{
					string customLordText = CustomisationFileManager.Instance.GetCustomLordText(name.ToLower(), 34 + subType);
					if (customLordText != null && customLordText != "")
					{
						return customLordText;
					}
				}
	...
	public const string MP_VERSION = "29";
	...
	private Action MapSendDelegate;
	...
	private Action MapProgressDelegate;
	...
	private ConcurrentQueue<MessageData> threadedMessages = new ConcurrentQueue<MessageData>();

	private Dictionary<CSteamID, ImageSource> _Cache = new Dictionary<CSteamID, ImageSource>();

	private ImageSource _localUserAvatar;

	private ImageSource _localUserCoatOfArms;

	private Callback<AvatarImageLoaded_t> m_AvatarLoadedRequested;
	...
	private void kickPlayerFromGame(MPGameMember kickMember, bool forceKickFromHost = false)
	{
	...
	public void GetLobbies(int defaultMatchmaking, Action lobbyListCompletedelegate)
	{
		...
		SteamMatchmaking.AddRequestLobbyListStringFilter("version", "29", ELobbyComparison.k_ELobbyComparisonEqual);
	...
	private void CreateLobbyResult(LobbyCreated_t param, bool bIOFailure)
	{
		if (param.m_eResult == EResult.k_EResultOK)
		{
			...
			SteamMatchmaking.SetLobbyData(cSteamID, "aiplayers", "0");
			SteamMatchmaking.SetLobbyData(cSteamID, "type", activeLobby.gameTypeCoop);
			...
			SteamMatchmaking.SetLobbyData(cSteamID, "version", "29");
			SteamMatchmaking.SetLobbyData(cSteamID, "country", SteamUtils.GetIPCountry());
			SteamMatchmaking.SetLobbyData(cSteamID, "closed", "0");
			SteamMatchmaking.SetLobbyData(cSteamID, "start", "");
			if (activeLobby.coopTrailGame)
			{
				SteamMatchmaking.SetLobbyData(cSteamID, "cooptrail", activeLobby.coopTrailID.ToString());
			}
			else
			{
				SteamMatchmaking.SetLobbyData(cSteamID, "cooptrail", "-1");
			}
			if (lastLobbyMode == 2)
			{
	...
	public void ChangeLobbyType(int _lobbyMode)
	{
		...
		SteamMatchmaking.SetLobbyType(activeLobby.id, eLobbyType);
	}
		...
		if (SteamManager.Initialized)
		{
			if (SteamApps.GetLaunchCommandLine(out var pszCommandLine, 260) > 0)
			{
				...
				if (array != null && array.Length != 0)
				{
					...
					{
						string text = array[i];
						if (text.ToLowerInvariant() == "+connect_lobby" && i + 1 < array.Length)
						{
							inviteLobbyID = EditorDirector.getuLongFromString(array[i + 1]);
							PendingMPLobby = true;
							break;
						}
						if (text.ToLowerInvariant() == "-skipvid")
						{
	...
	public void AutoJoinPendingLobby(ref MPLobby joiningLobby, Action lobbyJoinedDelegate, Action<string, string, int> lobbyChatDelegate)
	{
		...
		MPLobby mPLobby = new MPLobby
		{
			...
			AIPlayers = SteamMatchmaking.GetLobbyData(cSteamID, "aiplayers"),
			gameTypeCoop = SteamMatchmaking.GetLobbyData(cSteamID, "type"),
			settings = SteamMatchmaking.GetLobbyData(cSteamID, "settings"),
			country = SteamMatchmaking.GetLobbyData(cSteamID, "country"),
			crc = SteamMatchmaking.GetLobbyData(cSteamID, "crc"),
			setTeams = SteamMatchmaking.GetLobbyData(cSteamID, "teams"),
			isHost = false
		};
		mPLobby.coopTrailGameSetup(SteamMatchmaking.GetLobbyData(cSteamID, "cooptrail"));
		lobbies.Add(mPLobby);
	...
	private void JoinLobbyResult(LobbyEnter_t param, bool bIOFailure)
	{
		...
		{
			MPLobby mPLobby = ((autoJoinLobby == null || !(autoJoinLobby.id == cSteamID)) ? FindLobby(cSteamID) : autoJoinLobby);
			if ("29" == SteamMatchmaking.GetLobbyData(cSteamID, "version"))
			{
				if (mPLobby != null)
				{
					if (mPLobby.maxPlayers == "")
					{
						mPLobby.gameName = SteamMatchmaking.GetLobbyData(cSteamID, "name");
						mPLobby.mapName = SteamMatchmaking.GetLobbyData(cSteamID, "map");
						mPLobby.mapFileName = SteamMatchmaking.GetLobbyData(cSteamID, "mapfile");
						mPLobby.maxPlayers = SteamMatchmaking.GetLobbyData(cSteamID, "max");
						mPLobby.AIPlayers = SteamMatchmaking.GetLobbyData(cSteamID, "aiplayers");
						mPLobby.gameTypeCoop = SteamMatchmaking.GetLobbyData(cSteamID, "type");
						mPLobby.settings = SteamMatchmaking.GetLobbyData(cSteamID, "settings");
						mPLobby.country = SteamMatchmaking.GetLobbyData(cSteamID, "country");
						mPLobby.crc = SteamMatchmaking.GetLobbyData(cSteamID, "crc");
						mPLobby.setTeams = SteamMatchmaking.GetLobbyData(cSteamID, "teams");
						mPLobby.coopTrailGameSetup(SteamMatchmaking.GetLobbyData(cSteamID, "cooptrail"));
					}
					CreateShareCode(ulSteamIDLobby, mPLobby.coopTrailGame, mPLobby.coopTrailID);
					mPLobby.numLobbyMembers = SteamMatchmaking.GetNumLobbyMembers(cSteamID);
					bool flag = false;
					for (int i = 0; i < mPLobby.numLobbyMembers; i++)
					{
						if (SteamMatchmaking.GetLobbyMemberByIndex(cSteamID, i) == SteamUser.GetSteamID())
						{
							flag = true;
							break;
						}
	...
	public void SetCoopTrailProgress(int trailID, int[] progress, int selectedMission, int fullProgress, bool orderswapped)
	{
		...
		if (activeLobby.isHost)
		{
			...
			SteamMatchmaking.SetLobbyData(pchValue: (!orderswapped) ? (text + selectedMission + "," + fullProgress + ",0") : (text + selectedMission + "," + fullProgress + ",1"), steamIDLobby: activeLobby.id, pchKey: "cooptrail");
		}
	}
	...
	public bool GetActiveLobbyMembers(bool coopGame = false)
	{
		if (activeLobby != null)
		{
			_ = activeLobby.id;
			...
			if (coopGame && num > 2)
			{
				if (activeLobby.isHost)
				{
					for (int i = 2; i < num; i++)
					{
						...
						SteamMatchmaking.SetLobbyData(activeLobby.id, "kick", lobbyMemberByIndex.ToString());
					}
				}
				else
				{
	...
	public bool RefreshLobbyList(ref EngineInterface.MultiplayerSetupData MPSetupData, ref bool refreshTeams, ref bool settingsChanged, bool coopgame = false)
	{
		...
		if (activeLobby != null && !activeLobby.isHost)
		{
			...
			if (activeLobby.coopTrailGame)
			{
				for (int i = 0; i < 8; i++)
				{
					array2[i] = MPSetupData.start_keep_location_order[i];
				}
				for (int j = 0; j < 3; j++)
				{
					array[j] = MPSetupData.MP_BuildingsAvailable[j];
				}
			}
			...
			if (activeLobby.coopTrailGame)
			{
				for (int k = 0; k < 3; k++)
				{
					MPSetupData.MP_BuildingsAvailable[k] = array[k];
				}
				settingsChanged = false;
				for (int l = 0; l < 8; l++)
				{
					if (array2[l] != MPSetupData.start_keep_location_order[l])
					{
	...
	private void HandleIncomingMessage(LobbyChatMsg_t callback)
	{
		...
		if (lobbyChatEntry > 0)
		{
			...
			ChatHandle(message, pSteamIDUser);
		}
	}
	...
	private void EncodeAIVDataAddString(List<byte> dataList, string str)
	{
	...
	private void DecodeAIVData(byte[] data)
	{
		int offset = 0;
	...
	private string DecodeAIVDataString(byte[] data, ref int offset)
	{
	...
	public unsafe void SendCustomInfo(MPLobbyMember member, byte[] data)
	{
		...
		fixed (byte* ptr = data)
		{
			...
			EResult eResult = SteamNetworkingMessages.SendMessageToUser(ref identityRemote, pubData, (uint)data.Length, nSendFlags, 6);
	...
	private byte[] createMapSendData(int mode, int crc, byte[] data)
	{
	...
	private byte[] decodeMapData(byte[] message, ref int mode, ref int crc)
	{
	...
	private unsafe void SendMapInternal(CSteamID targetID, byte[] mapData, string mapFileName)
	{
		...
		int nSendFlags = 40;
		int num = (int)EngineInterface.crc(mapData);
		byte[] array = createMapSendData(0, num, BitConverter.GetBytes(mapData.Length));
		fixed (byte* ptr = array)
		{
			IntPtr pubData = (IntPtr)ptr;
			EResult eResult = SteamNetworkingMessages.SendMessageToUser(ref identityRemote, pubData, (uint)array.Length, nSendFlags, 1);
			if (eResult != EResult.k_EResultOK)
			{
				Debug.Log(eResult);
			}
		}
		byte[] array2 = createMapSendData(1, num, Encoding.UTF8.GetBytes(mapFileName));
		fixed (byte* ptr2 = array2)
		{
			IntPtr pubData2 = (IntPtr)ptr2;
			EResult eResult2 = SteamNetworkingMessages.SendMessageToUser(ref identityRemote, pubData2, (uint)array2.Length, nSendFlags, 1);
		...
		int num2;
		for (int i = 0; i < mapData.Length; i += num2)
		{
			MapSendQueueItem mapSendQueueItem = new MapSendQueueItem();
			mapSendQueueItem.NetID = identityRemote;
			num2 = Math.Min(250000, mapData.Length - i);
			mapSendQueueItem.data = new byte[num2 + 8];
			Array.Copy(BitConverter.GetBytes(2), 0, mapSendQueueItem.data, 0, 4);
			Array.Copy(BitConverter.GetBytes(num), 0, mapSendQueueItem.data, 4, 4);
			Array.Copy(mapData, i, mapSendQueueItem.data, 8, num2);
	...
	private unsafe void receiveLobbyMessages()
	{
		...
		for (int i = 0; i < num; i++)
		{
			SteamNetworkingMessage_t steamNetworkingMessage_t = System.Runtime.InteropServices.Marshal.PtrToStructure<SteamNetworkingMessage_t>(array[i]);
			byte[] array2 = new byte[steamNetworkingMessage_t.m_cbSize];
			System.Runtime.InteropServices.Marshal.Copy(steamNetworkingMessage_t.m_pData, array2, 0, array2.Length);
			...
			if (mode == 0)
			{
				...
				{
					lastCrc = crc;
					incomingSize = BitConverter.ToInt32(array3);
				...
				case 2:
				{
					...
					if (incomingOfset == incomingSize)
					{
						if (lastCrc != 0 && lastReceivedFileName.Length > 0)
						{
							...
							{
								File.WriteAllBytes(System.IO.Path.Combine(ConfigSettings.GetUserMapsPath(), lastReceivedFileName + ".map"), receiveBuffer);
		...
		if (!activeLobby.isHost)
		{
			...
			for (int j = 0; j < num; j++)
			{
				SteamNetworkingMessage_t steamNetworkingMessage_t2 = System.Runtime.InteropServices.Marshal.PtrToStructure<SteamNetworkingMessage_t>(array[j]);
				byte[] array4 = new byte[steamNetworkingMessage_t2.m_cbSize];
				System.Runtime.InteropServices.Marshal.Copy(steamNetworkingMessage_t2.m_pData, array4, 0, array4.Length);
				...
				if (headerFromMpSaveFileName != null && (headerFromMpSaveFileName.xPlaySaveChecksum ^ headerFromMpSaveFileName.xPlaySaveTime) == num3)
				{
					...
					{
						IntPtr pubData = (IntPtr)ptr;
						EResult eResult = SteamNetworkingMessages.SendMessageToUser(ref identityRemote, pubData, (uint)array4.Length, nSendFlags, 21);
						if (eResult != EResult.k_EResultOK)
						{
							Debug.Log(eResult);
						}
			...
			for (int k = 0; k < num; k++)
			{
				...
				SteamNetworkingIdentity identityRemote2 = System.Runtime.InteropServices.Marshal.PtrToStructure<SteamNetworkingMessage_t>(array[k]).m_identityPeer;
				SteamNetworkingMessage_t.Release(array[k]);
				fixed (byte* ptr2 = array5)
				{
					IntPtr pubData2 = (IntPtr)ptr2;
					EResult eResult2 = SteamNetworkingMessages.SendMessageToUser(ref identityRemote2, pubData2, (uint)array5.Length, nSendFlags, 32);
					if (eResult2 != EResult.k_EResultOK)
					{
						Debug.Log(eResult2);
					}
			...
			for (int l = 0; l < num; l++)
			{
				SteamNetworkingMessage_t steamNetworkingMessage_t3 = System.Runtime.InteropServices.Marshal.PtrToStructure<SteamNetworkingMessage_t>(array[l]);
				byte[] array6 = new byte[steamNetworkingMessage_t3.m_cbSize];
				System.Runtime.InteropServices.Marshal.Copy(steamNetworkingMessage_t3.m_pData, array6, 0, array6.Length);
		...
		for (int m = 0; m < num; m++)
		{
			SteamNetworkingMessage_t steamNetworkingMessage_t4 = System.Runtime.InteropServices.Marshal.PtrToStructure<SteamNetworkingMessage_t>(array[m]);
			byte[] array7 = new byte[steamNetworkingMessage_t4.m_cbSize];
			System.Runtime.InteropServices.Marshal.Copy(steamNetworkingMessage_t4.m_pData, array7, 0, array7.Length);
		...
		{
			_ = new byte[0];
			SteamNetworkingIdentity identityPeer = System.Runtime.InteropServices.Marshal.PtrToStructure<SteamNetworkingMessage_t>(array[n]).m_identityPeer;
	...
	public void StartGame(EngineInterface.MultiplayerSetupData MPSetupData, FileHeader map, int coopTrailID, int coopMissionID)
	{
		...
		foreach (MPLobbyMember member2 in activeLobby.members)
		{
			...
			mPGameMember.SNI.SetSteamID(member2.id);
	...
	private void HandleGameIncomingConnection(SteamNetworkingMessagesSessionRequest_t callback)
	{
		...
		if (activeLobby != null && identityRemote.GetSteamID() == SteamMatchmaking.GetLobbyOwner(activeLobby.id))
		{
			SteamNetworkingMessages.AcceptSessionWithUser(ref identityRemote);
		}
	}
	...
	private unsafe void SendGameData(MPGameMember target, byte[] dataToSend, bool instantMessage = false)
	{
		...
		{
			IntPtr pubData = (IntPtr)ptr;
			if (SteamNetworkingMessages.SendMessageToUser(ref identityRemote, pubData, (uint)dataToSend.Length, nSendFlags, 2) != EResult.k_EResultOK)
			{
				target.errorCount++;
				return;
			}
	...
	public void flushGameMessages()
	{
		...
		for (int i = 0; i < num; i++)
		{
			System.Runtime.InteropServices.Marshal.PtrToStructure<SteamNetworkingMessage_t>(array[i]);
			SteamNetworkingMessage_t.Release(array[i]);
		}
	}
	...
	public void ReceiveGameMessages(bool fromThread = false)
	{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			messageData.fromMember = findMember(steamNetworkingMessage_t.m_identityPeer);
			SteamNetworkingMessage_t.Release(array[i]);
	...
	private bool processMessage(MPData data, MPGameMember fromMember, bool fromThread)
	{
		if (fromMember == null)
		{
			if (data.packetType == 2)
			{
				...
				int num = BitConverter.ToInt32(data.data);
		...
		case 2:
		{
			...
			int mPRandSeed = BitConverter.ToInt32(data.data);
			if (mapLoaded)
			{
		...
		case 6:
			...
			{
				int num4 = BitConverter.ToInt32(data.data);
		...
		case 7:
		{
			...
			int playerID = BitConverter.ToInt32(data.data);
			MPGameMember player = getPlayer(playerID);
		...
		case 8:
			...
			break;
		case 10:
			CoopContinuationLobbyID = BitConverter.ToUInt64(data.data);
			break;
		}
		return true;
	}
	...
	private void CreateShareCode(ulong lobbyID, bool coopGame = false, int coopTrailID = -1)
	{
	...
	public ImageSource GetLocalAvatar()
	{
		...
		if (ConfigSettings.Settings_UseSteamAvatar)
		{
			if (_localUserAvatar != null)
			{
				return _localUserAvatar;
			}
		}
		else if (_localUserCoatOfArms != null)
		{
	...
	private void OnAvatarRequested(AvatarImageLoaded_t param)
	{
	...
	public void CreateCoAAvatar(CSteamID steamID, Avatars.AvatarDesign ad)
	{
		if (!_Cache.ContainsKey(steamID))
		{
			ImageSource avatarTexture = Avatars.Instance.GetAvatarTexture(ad);
	...
	public void RequestUserAvatar(CSteamID steamID, bool selfCall = false)
	{
		try
		{
			...
			case -1:
				...
				{
					m_AvatarLoadedRequested = Callback<AvatarImageLoaded_t>.Create(OnAvatarRequested);
				}
				return;
			}
			...
			int num = 12;
			Texture2D texture2D = new Texture2D((int)pnWidth + num * 2, (int)pnHeight + num * 2, UnityEngine.TextureFormat.RGBA32, mipChain: false, linear: false);
			byte[] array2 = new byte[(pnWidth + num * 2) * (pnHeight + num * 2) * 4];
			for (uint num2 = 0u; num2 < pnHeight; num2++)
			{
				for (uint num3 = 3u; num3 < pnWidth * 4; num3 += 4)
				{
					uint num4 = num3 / 4;
					if (num4 > pnWidth / 2)
					{
					...
					if (num5 > pnHeight / 2)
					{
						num5 = pnHeight - num5 - 1;
					}
					if (num4 < 12 || num5 < 12)
					{
						uint num6 = Math.Min(num4, num5);
						array[num3 + num2 * pnWidth * 4] = (byte)((int)array[num3 + num2 * pnWidth * 4] * num6 / 12);
					}
					if (array[num3 + num2 * pnWidth * 4] != byte.MaxValue)
					{
						byte b = array[num3 + num2 * pnWidth * 4];
						array[num3 - 1 + num2 * pnWidth * 4] = (byte)(array[num3 - 1 + num2 * pnWidth * 4] * b / 255);
						array[num3 - 2 + num2 * pnWidth * 4] = (byte)(array[num3 - 2 + num2 * pnWidth * 4] * b / 255);
						array[num3 - 3 + num2 * pnWidth * 4] = (byte)(array[num3 - 3 + num2 * pnWidth * 4] * b / 255);
					}
				}
			}
			int num7 = (int)(pnWidth + num * 2);
			for (int i = 0; i < pnHeight; i++)
			{
				for (int j = 0; j < pnWidth * 4; j++)
				{
					array2[j + num * 4 + (pnHeight - i - 1 + num) * num7 * 4] = array[j + i * pnWidth * 4];
				}
			}
			texture2D.LoadRawTextureData(array2);
			texture2D.Apply();
			TextureSource textureSource = new TextureSource(texture2D);
			if (steamID == SteamUser.GetSteamID())
			{
				_localUserAvatar = textureSource;
			}
			else
			{
				_Cache.Add(steamID, textureSource);
			}
