...
public class CustomisationFileManager
{
	...
	public class CustomMediaData
	{
		...
		public string[] tags = new string[41];
	}
	...
	private static readonly CustomisationFileManager instance;

	public bool filesChanged;

	private Dictionary<int, CustomLord> extendedLords = new Dictionary<int, CustomLord>();

	private Dictionary<string, CustomLord> customLords = new Dictionary<string, CustomLord>();

	private Dictionary<string, CustomMediaData> customMediaData = new Dictionary<string, CustomMediaData>();
	...
	private string[] mediaTags = new string[41]
	{
		...
		"team_losing", "will_send_troops", "will_attack_enemy", "nickname1", "nickname2", "nickname3", "nickname4", "nickname5", "nickname6", "nickname7",
		"nickname8"
	};

	private FileSystemWatcher extendedLordWatcher;

	private FileSystemWatcher customLordWatcher;

	private bool watchersCreated;
	...
	private CustomisationFileManager()
	{
	}
	...
	public void BuildFileLists()
	{
				...
				if (listOfSubscribedItemsPaths != null)
				{
					foreach (string item in listOfSubscribedItemsPaths)
					{
						...
						foreach (string text in directories)
						{
							...
							{
								Debug.Log("Unknown Workshop Lord : " + lordTypeFromName);
							}
						}
					}
				}
		...
		if (Directory.Exists(userCustomMediaPath))
		{
			...
			try
			{
				...
				{
					string text4 = System.IO.Path.Combine(text3, "text.txt");
	...
	private void BuildExtendedLordDirectory(string subdirectory)
	{
	...
	private void BuildCustomLordDirectory(string subdirectory, bool isWorkshop = false)
	{
		...
		if (!isWorkshop && File.Exists(System.IO.Path.Combine(subdirectory, directoryInfo.Name + ".data")))
		{
	...
	private void ProcessExtendedLordFile(string file, string realFileName, CustomLord customLord, bool workshop = false)
	{
		file = file.Replace('/', '\\');
		...
		case ".aivjson":
		{
			...
			try
			{
				...
				CustomAIV customAIV = new CustomAIV();
				customAIV.AIVName = System.IO.Path.GetFileNameWithoutExtension(file);
				customAIV.path = System.IO.Path.GetDirectoryName(file);
				customAIV.lordType = customLord.lordType;
		...
		case ".lordjson":
		{
			...
			try
			{
				...
				CustomLordConfig customLordConfig = new CustomLordConfig();
				customLordConfig.name = System.IO.Path.GetFileNameWithoutExtension(file);
				customLordConfig.path = System.IO.Path.GetDirectoryName(file);
		...
		case ".png":
			try
			{
				if (!(System.IO.Path.GetFileName(file).ToLower() == "avatar.png") || !File.Exists(file))
				{
					break;
				}
				...
				if (customLord.image != null)
				{
					if (customLord.image.Width == 144f && customLord.image.Height == 144f)
					{
	...
	private void AddCustomMediaText(string subDirectory, string textPath)
	{
		try
		{
			string name = new DirectoryInfo(subDirectory).Name;
			...
			for (int l = 0; l < 8; l++)
			{
				if (customMediaData.tags[33 + l] == null || customMediaData.tags[33 + l] == "")
				{
					customMediaData.tags[33 + l] = Translate.Instance.lookUpText(Enums.eTextSections.TEXT_CUSTOMISATION, 23 + l);
				}
				else if (customMediaData.tags[33 + l].Length > 20)
				{
					customMediaData.tags[33 + l] = customMediaData.tags[33 + l].Substring(0, 20);
				}
			}
	...
	private ulong createCRC(string fileName, short[] data)
	{
	...
	private ulong createCRC(string fileName, ref EngineInterface.AILordConfigTransferData td)
	{
		uint num = EngineInterface.crc(Encoding.UTF8.GetBytes(fileName));
	...
	private void ProcessCustomLordFile(string file, string realFileName)
	{
	}

	private int getLordTypeFromName(string lordName)
	{
	...
	private void CreateWatchers()
	{
		if (!watchersCreated)
		{
			...
			extendedLordWatcher.NotifyFilter = NotifyFilters.Attributes | NotifyFilters.CreationTime | NotifyFilters.DirectoryName | NotifyFilters.FileName | NotifyFilters.LastAccess | NotifyFilters.LastWrite | NotifyFilters.Security | NotifyFilters.Size;
			...
			customLordWatcher.NotifyFilter = NotifyFilters.Attributes | NotifyFilters.CreationTime | NotifyFilters.DirectoryName | NotifyFilters.FileName | NotifyFilters.LastAccess | NotifyFilters.LastWrite | NotifyFilters.Security | NotifyFilters.Size;
