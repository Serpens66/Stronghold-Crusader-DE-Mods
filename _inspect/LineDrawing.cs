...
public class LineDrawing : MonoBehaviour
{
	...
	public debugBox[] debugBoxArray;

	public debugLine[] debugLineArray;

	public debugLine[] hudLineArray;

	public const int MAXDebugBoxes = 2000;

	public const int MAXDebugLines = 10000;

	public const int MAXHUDLines = 50;
	...
	public void addADebugBox(float x, float y, float w, float h)
	{
		...
		if (freeDebugBox > -1)
		{
			...
			debugBoxArray[freeDebugBox].drawnLine.color = Color32.op_Implicit(Color.yellow);
			debugBoxArray[freeDebugBox].drawnLine.points3.Clear();
			createDebugBoxLine(freeDebugBox);
			((Transform)debugBoxArray[freeDebugBox].drawnLine.rectTransform).SetParent(boxParent.transform);
		}
	}

	public void createDebugBoxLine(int thisDebugBox)
	{
	...
	public void addADebugLine(float x1, float y1, float x2, float y2)
	{
		...
		if (freeDebugLine > -1)
		{
			...
			debugLineArray[freeDebugLine].drawnLine.color = Color32.op_Implicit(Color.cyan);
			debugLineArray[freeDebugLine].drawnLine.points3.Clear();
			createDebugLineLine(freeDebugLine);
			((Transform)debugLineArray[freeDebugLine].drawnLine.rectTransform).SetParent(lineParent.transform);
		}
	}

	public void createDebugLineLine(int thisDebugLine)
	{
		//IL_009b: Unknown result type (might be due to invalid IL or missing references)
	...
	public void repositionHUDLine(int thisHUDLine, int x1, int y1, int x2, int y2)
	{
		...
		if (thisHUDLine > 0)
		{
			...
			PerfectPixelWithZoom.instance.returnCamaraToCurrentSize();
			((Transform)hudLineArray[thisHUDLine].drawnLine.rectTransform).SetParent(hudLineParent.transform);
		}
	}

	public void createHUDLine(int thisHUDLine)
	{
		//IL_009b: Unknown result type (might be due to invalid IL or missing references)
		//IL_00d4: Unknown result type (might be due to invalid IL or missing references)
