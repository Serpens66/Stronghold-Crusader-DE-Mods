...
public class OnScreenText
{
	...
	private float WhoOwns_Opacity;

	private int WhoOwns_Data1;

	private int WhoOwns_Data2;

	private int WhoOwns_Data3;

	private int WhoOwns_Data4;

	private int WhoOwns_Data5;

	private DateTime nextCartFrame = DateTime.MinValue;

	private int cartFrame;

	private const int BarLength = 494;

	private int lastSoTRank = -1;
			...
			if (wasTurnedOnOrChanged)
			{
				...
				for (int num2 = 1; num2 < 9; num2++)
				{
					...
					if (playerName.Length > 0 && num2 != GameData.Instance.playerID)
					{
						...
						UnityEngine.Color color = MPTeamColours[SpriteMapping.remapColours[SpriteMapping.RemapMPLoadedColour(num2)]];
		...
		if (oST3 != null)
		{
			...
			for (int num4 = 0; num4 < 8; num4++)
			{
				...
				if ((oST3.data2 & (1 << num4)) != 0)
				{
					...
					UnityEngine.Color color2 = MPTeamColours[SpriteMapping.remapColours[SpriteMapping.RemapMPLoadedColour(num5)]];
					...
					MainViewModel.Instance.OST_MP_Game_Over_Row_Vis[num5] = Visibility.Visible;
				}
				else
				{
					MainViewModel.Instance.OST_MP_Game_Over_Row_Vis[num5] = Visibility.Collapsed;
				}
			}
		}
		else if (wasTurnedOff)
	...
	public static string getComputerName(int computerOpponent, int computerName)
	{
		...
		case 37:
		{
			...
			{
				string text = Translate.Instance.lookUpText(Enums.eTextSections.TEXT_CUSTOMISATION, 23 + computerName);
				if (CustomisationFileManager.CustomMediaExists)
				{
					string customLordText = CustomisationFileManager.Instance.GetCustomLordText(MainViewModel.Instance.HUDIngameMenu.restartSkirmishMapInfo.aivs[computerOpponent - 30].lordName.ToLower(), 34 + computerName);
					if (customLordText != null && customLordText != "")
					{
						text = customLordText;
					}
				}
				return MapFileManager.SplitCustomTrailName(MainViewModel.Instance.HUDIngameMenu.restartSkirmishMapInfo.aivs[computerOpponent - 30].lordName) + " " + text;
			}
			if (MainViewModel.Instance.HUDIngameMenu.restartMPInfo == null)
			{
				break;
			}
			string text2 = Translate.Instance.lookUpText(Enums.eTextSections.TEXT_CUSTOMISATION, 23 + computerName);
			if (CustomisationFileManager.CustomMediaExists)
			{
				string customLordText2 = CustomisationFileManager.Instance.GetCustomLordText(MainViewModel.Instance.HUDIngameMenu.restartMPInfo.LordNames[computerOpponent - 30].ToLower(), 34 + computerName);
				if (customLordText2 != null && customLordText2 != "")
				{
					text2 = customLordText2;
				}
			}
			return MapFileManager.SplitCustomTrailName(MainViewModel.Instance.HUDIngameMenu.restartMPInfo.LordNames[computerOpponent - 30]) + " " + text2;
		}
		}
		return "";
	}

	private void UpdateSandsOfTimer()
	{
		int seconds = 0;
		...
		case Enums.SandsRanks.Prince:
		{
			...
			if (rank != lastSoTRank)
			{
				...
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_1.Opacity = 1f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_2.Opacity = 0.5f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_3.Opacity = 0.5f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_4.Opacity = 0.5f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_5.Opacity = 0.5f;
		...
		case Enums.SandsRanks.Champion:
		{
			...
			if (rank != lastSoTRank)
			{
				...
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_4.Source = GameData.GetSandsOfTimeImage(Enums.SandsRanks.Tribesman, greyedImage: true, large: false, small: true);
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_5.Source = GameData.GetSandsOfTimeImage(Enums.SandsRanks.Peasant, greyedImage: true, large: false, small: true);
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_1.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_2.Opacity = 1f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_3.Opacity = 0.5f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_4.Opacity = 0.5f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_5.Opacity = 0.5f;
				MainViewModel.Instance.OST_same_time_prince = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Prince));
		...
		case Enums.SandsRanks.Warrior:
		{
			...
			if (rank != lastSoTRank)
			{
				...
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_5.Source = GameData.GetSandsOfTimeImage(Enums.SandsRanks.Peasant, greyedImage: true, large: false, small: true);
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_1.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_2.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_3.Opacity = 1f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_4.Opacity = 0.5f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_5.Opacity = 0.5f;
				MainViewModel.Instance.OST_same_time_prince = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Prince));
				MainViewModel.Instance.OST_same_time_champion = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Champion));
		...
		case Enums.SandsRanks.Tribesman:
		{
			...
			if (rank != lastSoTRank)
			{
				...
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_1.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_2.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_3.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_4.Opacity = 1f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_5.Opacity = 0.5f;
				MainViewModel.Instance.OST_same_time_prince = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Prince));
				MainViewModel.Instance.OST_same_time_champion = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Champion));
				MainViewModel.Instance.OST_same_time_warrior = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Warrior));
		...
		case Enums.SandsRanks.Peasant:
		{
			...
			if (rank != lastSoTRank)
			{
				...
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_2.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_3.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_4.Opacity = 0.75f;
				MainViewModel.Instance.IngameUI.RefOST_Sands_Target_Bar_5.Opacity = 1f;
				MainViewModel.Instance.OST_same_time_prince = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Prince));
				MainViewModel.Instance.OST_same_time_champion = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Champion));
				MainViewModel.Instance.OST_same_time_warrior = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Warrior));
				MainViewModel.Instance.OST_same_time_tribesman = GameData.GetTimeString(GameData.GetSandsOfTimeRankTime(seconds, Enums.SandsRanks.Tribesman));
