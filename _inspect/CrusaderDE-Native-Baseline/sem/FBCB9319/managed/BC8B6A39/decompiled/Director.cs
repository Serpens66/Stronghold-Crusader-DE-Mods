...
public class Director : MonoBehaviour
{
	...
	private double pendingTimerChange;
	...
	private bool waitCursorSet;

	private int currentCursorType = -1;

	private bool multiplayerGame;

	private bool skirmishModeGame;

	private bool wasSkirmishModeGame;
	...
	private void UpdateFixedTimer(double timer)
	{
		pendingTimerChange = timer;
	}

	private void MyFixedUpdate(object state)
	{
	...
	public void IncreaseFrameRate()
	{
		...
		if (num2 < (double)num)
		{
			num2 = ((!(num2 <= 95.0)) ? (num2 + 25.0) : (num2 + 5.0));
			if (num2 > (double)num)
			{
				num2 = num;
			}
	...
	public void DecreaseFrameRate()
	{
		...
		if (num > 10.0)
		{
			num = ((!(num >= 125.0)) ? (num - 5.0) : (num - 25.0));
			if (num < 10.0)
			{
				num = 10.0;
			}
	...
	public void setCursor(int cursorType, bool force = false)
	{
		...
		if (ConfigSettings.Settings_CursorStyle == 0)
		{
			...
			{
			case 0:
				UnityEngine.Cursor.SetCursor(swordCursor32, new Vector2(2f, 1f), CursorMode.Auto);
				break;
			case 1:
				UnityEngine.Cursor.SetCursor(handCursor32, new Vector2(1f, 1f), CursorMode.Auto);
				break;
			case 2:
				UnityEngine.Cursor.SetCursor(deleteCursor32, new Vector2(10f, 10f), CursorMode.Auto);
				break;
			case 3:
				UnityEngine.Cursor.SetCursor(deleteNotCursor32, new Vector2(10f, 10f), CursorMode.Auto);
				break;
			case 4:
				UnityEngine.Cursor.SetCursor(waitCursor32, new Vector2(16f, 16f), CursorMode.Auto);
				break;
			case 5:
				UnityEngine.Cursor.SetCursor(scimitarCursor32, new Vector2(1f, 1f), CursorMode.Auto);
				break;
			}
		}
		else if (ConfigSettings.Settings_CursorStyle == 3)
		{
			...
			case 3:
				...
				break;
			case 4:
				UnityEngine.Cursor.SetCursor(waitCursorX, new Vector2(16f, 16f), CursorMode.ForceSoftware);
				break;
			case 5:
				UnityEngine.Cursor.SetCursor(scimitarCursorX, new Vector2(1f, 1f), CursorMode.ForceSoftware);
				break;
			}
		}
		else if (ConfigSettings.Settings_CursorStyle == 2)
		{
			...
			{
			case 0:
				UnityEngine.Cursor.SetCursor(swordCursor, new Vector2(2f, 1f), CursorMode.ForceSoftware);
				break;
			case 1:
				UnityEngine.Cursor.SetCursor(handCursor, new Vector2(1f, 1f), CursorMode.ForceSoftware);
				break;
			case 2:
				UnityEngine.Cursor.SetCursor(deleteCursor, new Vector2(10f, 10f), CursorMode.ForceSoftware);
				break;
			case 3:
				UnityEngine.Cursor.SetCursor(deleteNotCursor, new Vector2(10f, 10f), CursorMode.ForceSoftware);
				break;
			case 4:
				UnityEngine.Cursor.SetCursor(waitCursor, new Vector2(16f, 16f), CursorMode.ForceSoftware);
				break;
			case 5:
				UnityEngine.Cursor.SetCursor(scimitarCursor, new Vector2(1f, 1f), CursorMode.ForceSoftware);
				break;
			}
		}
		else if (ConfigSettings.Settings_CursorStyle == 1)
		{
			...
			case 2:
			case 3:
				UnityEngine.Cursor.SetCursor(deleteOldCursor, new Vector2(4f, 4f), CursorMode.Auto);
				break;
			}
		}
	}
	...
	{
		StartCoroutine(ManageGameOver(state, screen, victory));
	}

	private IEnumerator ManageGameOver(int state, int screen, bool victory)
	{
	...
	{
		StartCoroutine(doGenericDelayCoroutine(action, seconds));
	}

	private IEnumerator doGenericDelayCoroutine(Action action, float seconds)
	{
	...
	{
		StartCoroutine(DoSignUp(emailAddress, success, showRequester, checkCall));
	}

	private IEnumerator DoSignUp(string emailAddress, Action success, bool showRequester, bool checkCall)
	{
		WWWForm wWWForm = new WWWForm();
		wWWForm.AddField("email", emailAddress);
		wWWForm.AddField("steamid", Platform_Multiplayer.Instance.GetLocalSteamID().ToString());
		wWWForm.AddField("lang", FatControler.locale);
		if (checkCall)
		{
			wWWForm.AddField("check", "true");
		}
		else
		{
			wWWForm.AddField("check", "false");
		}
		using UnityWebRequest www = UnityWebRequest.Post("https://login.strongholdkingdoms.com/sams/api/shcde/registerEmail.php", wWWForm);
	...
	private IEnumerator DoLoadImages()
	{
		...
		for (int imageID = 0; imageID < 109; imageID++)
		{
