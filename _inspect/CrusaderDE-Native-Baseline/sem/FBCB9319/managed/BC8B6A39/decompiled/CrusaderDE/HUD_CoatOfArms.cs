...
{
	private static bool OrdinaryTabSelected = true;

	private static bool MainColourSelected = true;

	private static bool ChargeColourLockedSelected = true;

	public static readonly DependencyProperty COA_UseSteamAvatarProperty = DependencyProperty.Register("COA_UseSteamAvatar", typeof(bool), typeof(HUD_CoatOfArms), new PropertyMetadata(null));

	private static HUD_CoatOfArms instance1 = null;

	private static HUD_CoatOfArms instance2 = null;

	private static HUD_CoatOfArms instance3 = null;

	private static HUD_CoatOfArms instance4 = null;

	private static HUD_CoatOfArms instance5 = null;

	private static Enums.AvatarItems[] OrdinariesDisplayOrder = new Enums.AvatarItems[91]
	{
	...
	private static Enums.AvatarItems[] ChargesDisplayOrder = new Enums.AvatarItems[90]
	{
		...
		Enums.AvatarItems.Item_SURGEON,
		Enums.AvatarItems.Item_BAIBARS,
		...
		Enums.AvatarItems.Item_VULTURE,
		...
		Enums.AvatarItems.Item_FOOLKING,
	...
	private static string[] chargeRolloverIDs = new string[93]
	{
	...
	private static Enums.AvatarItems[] ColourDisplayOrder = new Enums.AvatarItems[25]
	{
	...
	private static SolidColorBrush[] ColourSwabs = new SolidColorBrush[25]
	{
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 19, 19, 19)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 31, 62, 171)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 109, 54, 5)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 22, 22, 91)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 51, 28, 11)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 0, 100, 0)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 43, 20, 60)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 75, 9, 9)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 192, 97, 97)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 231, 180, 72)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 100, 141, 13)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 93, 93, 93)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 72, 150, 215)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 143, 195, 72)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, byte.MaxValue, 141, 0)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 212, 155, 209)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 114, 89, 164)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 239, 189, 73)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 178, 87, 24)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 229, 74, 160)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 130, 22, 130)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 195, 43, 43)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 0, 96, 96)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 37, 177, 140)),
		new SolidColorBrush(Color.FromArgb(byte.MaxValue, 181, 181, 181))
	};

	private static bool imagesCreate = false;

	public bool COA_UseSteamAvatar
	{
		get
		{
			return (bool)GetValue(COA_UseSteamAvatarProperty);
		}
		set
		{
			SetValue(COA_UseSteamAvatarProperty, value);
		}
	}
	...
	{
		NoesisUnity.LoadComponent(this, "Assets/GUI/XAMLResources/HUD_CoatOfArms.xaml");
	}
	...
	{
		MainViewModel.Instance.CoA_Main_Image = Avatars.Instance.GetAvatarTexture(ad, featherEdge: false);
	...
	{
		bool flag = false;
		if (instance1 != null && !flag && instance1.COA_UseSteamAvatar != ConfigSettings.Settings_UseSteamAvatar)
		{
			ConfigSettings.Settings_UseSteamAvatar = instance1.COA_UseSteamAvatar;
			flag = true;
		}
		if (instance2 != null && !flag && instance2.COA_UseSteamAvatar != ConfigSettings.Settings_UseSteamAvatar)
		{
			ConfigSettings.Settings_UseSteamAvatar = instance2.COA_UseSteamAvatar;
			flag = true;
		}
		if (instance3 != null && !flag && instance3.COA_UseSteamAvatar != ConfigSettings.Settings_UseSteamAvatar)
		{
			ConfigSettings.Settings_UseSteamAvatar = instance3.COA_UseSteamAvatar;
			flag = true;
		}
		if (instance4 != null && !flag && instance4.COA_UseSteamAvatar != ConfigSettings.Settings_UseSteamAvatar)
		{
			ConfigSettings.Settings_UseSteamAvatar = instance4.COA_UseSteamAvatar;
			flag = true;
		}
		if (instance5 != null && !flag && instance5.COA_UseSteamAvatar != ConfigSettings.Settings_UseSteamAvatar)
		{
	...
	public static void UpdateChargesAndOrdinaries()
	{
		if (!imagesCreate)
		{
			...
			foreach (Enums.AvatarItems item in chargesDisplayOrder)
			{
				...
				setCoACharge(num, Avatars.Instance.GetAvatarTexture(avatarDesign, featherEdge: false, UIListVariant: true));
				num++;
			}
			...
			foreach (Enums.AvatarItems background in chargesDisplayOrder)
			{
				...
				setCoAOrdinary(num, Avatars.Instance.GetAvatarTexture(avatarDesign, featherEdge: false));
				num++;
			}
		}
	}
		...
		case "Charge88":
		{
			...
			if (num4 < ChargesDisplayOrder.Length)
			{
				...
				if (num4 != ConfigSettings.Settings_AvatarCharge)
				{
					...
					MainViewModel.Instance.CoA_Main_Image = Avatars.Instance.GetAvatarTexture(ConfigSettings.getAvatar(), featherEdge: false);
					ConfigSettings.SaveSettings();
				}
			}
			break;
	...
	private static void SetColourSwabs()
	{
		for (int i = 0; i < 25; i++)
		{
	...
	public static void setCoACharge(int index, ImageSource tex)
	{
		...
		case 86:
			...
			break;
		case 87:
			MainViewModel.Instance.CoACharges87 = tex;
			break;
		case 88:
			MainViewModel.Instance.CoACharges88 = tex;
			break;
		case 89:
			MainViewModel.Instance.CoACharges89 = tex;
			break;
		}
	}
