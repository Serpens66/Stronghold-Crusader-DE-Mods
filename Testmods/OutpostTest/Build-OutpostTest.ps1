...
try {
    ...
    & (Join-Path $root 'Verify-OutpostTest.ps1')
    $msbuild = 'C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\MSBuild\Current\Bin\MSBuild.exe'
    & $msbuild (Join-Path $root 'tests\OutpostTest.Tests.csproj') /nologo /verbosity:minimal /p:Configuration=Debug "/p:ExtenderDir=$extender" "/p:GameDir=$game"
    if ($LASTEXITCODE -ne 0) { throw 'Test build failed.' }
    & (Join-Path $root 'tests\bin\OutpostTest.Tests.exe')
    if ($LASTEXITCODE -ne 0) { throw 'Tests failed.' }
    & $msbuild (Join-Path $root 'OutpostTest.csproj') /nologo /verbosity:minimal /p:Configuration=Debug "/p:ExtenderDir=$extender" "/p:GameDir=$game"
    ...
    foreach ($name in @('OutpostTest.dll', 'OutpostTest.pdb', 'info.json')) {
        ...
        if ((Get-OutpostFileHash (Join-Path $package $name)) -ne (Get-OutpostFileHash (Join-Path $destination $name))) {
            throw "Installation hash mismatch: $name"
        }
    }
    Write-Output "OutpostTest tests passed; installed in $destination"
    exit 0
} catch {
