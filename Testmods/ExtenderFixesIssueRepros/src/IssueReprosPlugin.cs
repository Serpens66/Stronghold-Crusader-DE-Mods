        ...
        private static void OnMapPostLoad(MapPostLoadEventArgs e)
        {
            ...
            try
            {
                ...
                for (int playerId = 1; playerId <= 8; playerId++)
                {
                    ...
                    bool stale = !hasPreferences && flags.Split(' ').Any(value => value.EndsWith("=True", StringComparison.Ordinal));
                    Log.LogInfo($"[FIXES-FLAGS] player={playerId} lord={lordName} hasPreferences={hasPreferences} {flags} result={(stale ? "REPRODUCED: enabled flag without current preferences" : "OBSERVED")}");
                }
            }
            catch (Exception ex)
            {
