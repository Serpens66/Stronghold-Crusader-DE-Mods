...
### Der verspätete Holzfällerstart stammt aus Vanillas implizitem Mindestscore
...
Die vollständige Rekonstruktion von `0x58020` und `Log_063.log` lösen den verbliebenen Widerspruch auf. Vanilla markiert besuchte Zellen vor den Prädikaten, reiht zulässige Nachbarn in seiner Originalreihenfolge ein und zählt jeden formalen Holzkandidaten mit `signed byte+04 < 6`, `signed byte+07 > 0` und `byte+13 == 0`. Danach berechnet es `signed(byte+07) * 5 - Elterntiefe * 3`. Ist `byte+06` ungleich null, wird ein positiver Score halbiert und ein nichtpositiver Score verdoppelt.

Der lokale Bestscore wird jedoch mit `-100` initialisiert und ein Kandidat nur bei strikt höherem Score übernommen. Sind formale Kandidaten vorhanden, aber alle Scores höchstens `-100`, bleibt das Ergebnis deshalb `(-1,-1)`. Der Kandidatenzähler ist dennoch positiv und der Fehl-Cooldown bleibt `0→0`. Genau dieses Muster zeigen die ergebnislosen aktiven Portal-/Durchbruchläufe. `byte+16`, `byte+13` und die Tiefengrenze 60 sind für diesen beobachteten Leerfall damit als Ursache zurückgenommen.

Der aktive Testpatch ersetzt ausschließlich während einer berechtigten `ActivePortal`- oder `ActiveBreach`-Holzsuche diesen lokalen Initialwert durch `INT_MIN`. Vanillas Originalsuche läuft genau einmal und behält Traversierung, Prädikate, Scoring, Bestkandidatenauswahl, Ergebnisfelder, Cooldowns, Nachsuche, Erreichbarkeitsprüfung, Validator und Konstruktion unverändert. Liegt Vanillas bester Kandidat bereits über `-100`, ändert sich das Ergebnis nicht; andernfalls kann Vanilla nun den tatsächlich besten formalen Kandidaten selbst übernehmen.
...
4. Eine torlos eingeschlossene KI bleibt als `PendingBreach` vollständig Vanilla. Normale Mauersegmente besitzen keinen Building-Record; der Schadenscallback muss sie daher über seine Tile-ID gegen die beim Kartenstart erfasste Mauerbaseline auflösen. Vanilla speichert ihre laufende Mauerhöhe in `HeightGrid`, die Grundhöhe in `DefaultHeightGrid` und den Eigentümer als nullbasierten Wert in `WallOwnerGrid`. Erst wenn ein Tile der konkret ausgewählten Baseline-Einfassung sein `IsWall`-Bit verliert und das zu genau diesem Tile gehörende stabile Innen-/Außenankerpaar in einer gemeinsamen positiven, von der Burg erreichbaren PCL liegt, darf die KI `ActiveBreach` werden. Eine dabei entstehende gemeinsame PCL ist ausdrücklich zulässig; bloße PCL-Neunummerierung, Schaden oder Toranimation reichen nicht.
...
Die frühere Vollanalyse jeder Wirtschaftssuche war für die Ursachenfindung nützlich, erzeugte auf der Mauerkarte aber erhebliche Last und ist nicht mehr Bestandteil des aktiven Fixpfads. Der Mod besitzt keine Unity-`Update`-/`LateUpdate`-/`FixedUpdate`-Methode, keinen Timer und keine periodische Polling-Schleife. Er installiert nur noch die einmalige Vorplatzierungsgrenze `0x50680`, den Altformattransfer `0xD4290`, den Rasterwriter `0x50720`, die vier Wirtschaftssuchen `0x575B0`/`0x57B80`/`0x58020`/`0x58950`, die drei für die Nachsuche nötigen Dispatcher `0x50F90`/`0x51270`/`0x51540` sowie den eng begrenzten Holzscore-Context-Hook `0x58057`. `0x55FE0` wird für den einmaligen Re-Census über einen validierten, statisch verwurzelten Delegate direkt aufgerufen.

Vorplatzierte Building- und Wall-Identitäten werden einmal vor der ersten AIV-Zuweisung erfasst. Danach markieren ausschließlich relevante Damage-, Bulldoze-, Delete-, Portal- und `0x50720`-Ereignisse die Topologie als geändert. Eine torlose KI wird erst beim nächsten echten Wirtschaftslauf anhand der verlorenen Baseline-Mauertiles und stabilen PCL-Anker geprüft; ist Vanillas PCL-Neuaufbau noch nicht abgeschlossen, bleibt sie unverändert `PendingBreach`. Gewöhnliche Gebäude erzeugen keine Inventar-, Raster- oder Routensnapshots. Projektionen und erreichbare PCLs werden pro Spieler und Topologierevision wiederverwendet; pro Suchlauf werden nur tatsächlich abweichende `byte+04`-Zellen in vorab angelegten Arrays gesichert und restauriert. Nach dem Cacheaufbau entstehen im normalen Suchhook keine verwalteten Allokationen. Verschachtelung, Vertragsfehler oder unvollständige Restaurierung schalten den Wirtschaftspfad prozessweit fail-closed auf Vanilla zurück.

Für den Holzfällerpfad ist die vollständige Vanilla-Kette festgehalten: `0x51540` prüft Holznachfrage und globalen Bestand, `0x58020` führt die Grobsuche und Originalbewertung aus, `0x58950` sucht anschließend eine vollständig freie Nachbarzelle, `0xC3BF0` prüft deren PCL-Erreichbarkeit über `0xE2610`, und erst danach ruft Vanilla `0x6D580` auf. Der eng begrenzte Score-Patch greift nur am Bestscore-Initialisierer bei `0x58057` ein und nur innerhalb des bereits aktiven spielerspezifischen Vorplatzierungs-Overlays. Er führt keine eigene Suche und keinen zweiten Suchdurchlauf ein.
