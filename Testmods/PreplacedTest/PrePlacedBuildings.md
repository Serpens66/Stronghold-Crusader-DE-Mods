...
Für den noch auffälligen späten Holzfällerstart ist die vollständige Vanilla-Kette nun festgehalten: `0x51540` prüft zunächst Holznachfrage und globalen Bestand, `0x58020` führt die Grobsuche aus, markiert die gewählte Zelle mit `byte+06=4`, `0x58950` sucht anschließend eine vollständig freie Nachbarzelle, `0xC3BF0` prüft deren PCL-Erreichbarkeit über `0xE2610`, und erst danach ruft Vanilla `0x6D580` auf. In `0x58020` ist die Kandidatenprüfung in den Expansionszweig eingebettet: effektiv gelten `signed byte+04 < 6`, `signed byte+07 > 0` und `byte+13 == 0`; das Scoring lautet `byte+07*5 - Tiefe*3` mit der `byte+06`-Anpassung. Der Start-Census `0x55FE0` verwendet dagegen für die Holzverfügbarkeit nur `signed byte+04 - signed byte+16 < 6`. Der Testmod korreliert beide Prädikate, native Besuchs- und Queuewerte, Ergebnis, Nachsuche, PCL-Prüfung und Konstruktion jetzt in kompakten semantischen Signaturen. Damit lässt sich im nächsten Lauf unterscheiden, ob ein verzögerter Holzfäller durch Census, Grobsuche, Nachsuche, Erreichbarkeit oder Konstruktion entsteht.
