...
rem The installed Script Extender 2.3.0 release is canonical; this variable is the explicit override.
if defined SHCDESE_EXTENDER_DIR set "GAME_SCRIPT_EXTENDER_DIR=%SHCDESE_EXTENDER_DIR%"
set "PLUGIN_NAME=KnightArmorAIBuyFixBackup_Serp"
...
pushd "%PROJECT_DIR%"
"%MSBUILD%" tests\KnightArmorAIBuyFixBackup.Tests.csproj /p:Configuration=Debug /p:ExtenderDir="%GAME_SCRIPT_EXTENDER_DIR%"
if errorlevel 1 goto build_failed_popd
"%PROJECT_DIR%tests\bin\KnightArmorAIBuyFixBackup.Tests.exe"
if not "%ERRORLEVEL%"=="0" goto build_failed_popd
popd

if exist "%LOCAL_PLUGIN_DIR%\" rmdir /S /Q "%LOCAL_PLUGIN_DIR%"
pushd "%PROJECT_DIR%"
"%MSBUILD%" KnightArmorAIBuyFixBackup.csproj /p:Configuration=Debug /p:GameDir="%GAME_DIR%" /p:ExtenderDir="%GAME_SCRIPT_EXTENDER_DIR%"
if errorlevel 1 goto build_failed_popd
popd

copy /Y "%PROJECT_DIR%info.json" "%LOCAL_PLUGIN_DIR%\info.json" >nul
if not exist "%LOCAL_PLUGIN_DIR%\KnightArmorAIBuyFixBackup.dll" goto package_failed
...
echo Knight Armor AI Buy Fix Backup built and installed successfully.
if "%NO_PAUSE%"=="0" pause
exit /b 0

:built_without_install
echo Knight Armor AI Buy Fix Backup built successfully. Installation skipped.
if "%NO_PAUSE%"=="0" pause
exit /b 0
