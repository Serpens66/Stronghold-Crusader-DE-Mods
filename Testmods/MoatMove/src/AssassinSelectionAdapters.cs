...
namespace MoatMove
{
    internal sealed unsafe partial class FriendlyMoatMovementRuntime
    {
        ...
        private readonly HookHandle<X64InlineHook>[] selectionCallHooks = {
            ...
            new HookHandle<X64InlineHook>(), new HookHandle<X64InlineHook>(),
            new HookHandle<X64InlineHook>(), new HookHandle<X64InlineHook>() };
        private static readonly int[] SelectionCallRvas = { 0x8D724, 0x8E2B8, 0x8E550, 0x8F325, 0xB7161, 0xB7321 };
        ...
        private void AddSelectionCallAdapters(HookTransaction transaction, ReadOnlySpan<byte> memory, ulong libraryBase)
        {
            selectionResultCallback = ObserveCursorTilePairFallbackSelection;
            ulong callback = unchecked((ulong)Marshal.GetFunctionPointerForDelegate(selectionResultCallback).ToInt64());
            for (int i = 0; i < SelectionCallRvas.Length; i++)
            {
                int rva = SelectionCallRvas[i]; string hex = SelectionCallBytes[i];
                var bytes = new byte[hex.Length / 2];
                for (int j = 0; j < bytes.Length; j++) bytes[j] = Convert.ToByte(hex.Substring(j * 2, 2), 16);
                ValidateExactBytes(memory, rva, bytes, "SE 2.6 selection call and complete displaced span");
                int expectedLength = bytes.Length;
                transaction.AddInline(selectionCallHooks[i], HookTarget.FromAddress(libraryBase + (uint)rva),
                    (asm, original, returnAddress) => {
                        if (returnAddress != libraryBase + (uint)rva + (uint)expectedLength ||
                            ...
                            throw new InvalidOperationException("SE selection call relocation contract changed.");
        ...
        private void ValidateSelectionCallAdapters(ulong libraryBase)
        {
            ...
            {
                var handle = selectionCallHooks[i];
                if (!handle.Success || !handle.IsInstalled || handle.Failure != null ||
                    ...
                    throw new InvalidOperationException("SE selection adapter installation mismatch.");
            }
        }

        private static void EmitSelectionCallAdapter(Assembler asm, ReadOnlySpan<Instruction> original, ulong callback)
        {
            ...
            asm.AddInstruction(original[0]); // Calls the live SE-owned entry exactly once.
            ...
            asm.mov(rcx, __qword_ptr[rsp + 0xD0]); asm.mov(rdx, __qword_ptr[rsp + 0x20]);
            asm.mov(rax, callback); asm.call(rax); asm.mov(__qword_ptr[rsp + 0x20], rax);
            ...
            asm.mov(rcx, __qword_ptr[rsp + 0x28]); asm.mov(rdx, __qword_ptr[rsp + 0x30]);
            asm.mov(r8, __qword_ptr[rsp + 0x38]); asm.mov(r9, __qword_ptr[rsp + 0x40]);
            asm.mov(r10, __qword_ptr[rsp + 0x48]); asm.mov(r11, __qword_ptr[rsp + 0x50]);
            asm.mov(rax, __qword_ptr[rsp + 0x20]); asm.lea(rsp, __qword_ptr[rsp + 0xE0]);
            ...
            for (int i = 1; i < original.Length; i++) asm.AddInstruction(original[i]);
        }
    }
}
