...
$manifest = Get-Content -LiteralPath (Join-Path $modDir 'info.json') -Raw | ConvertFrom-Json
if ($version.ToString() -ne "$($manifest.Version).0") { throw "Unexpected assembly version: $version" }
...
Write-Output "PASS installed MoatMove $($manifest.Version); DLL/PDB/manifest hashes match: $installedDir"
