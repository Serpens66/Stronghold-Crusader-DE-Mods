...
source = (root / 'Testmods/MoatMove/src/FriendlyMoatMovementRuntime.cs').read_text(encoding='utf-8-sig')
...
ladder_source = (root / 'Testmods/MoatMove/src/FriendlyMoatMovementRuntime.LadderAttackFix.cs').read_text(encoding='utf-8-sig')
...
recovery = (root / 'Testmods/MoatMove/src/NativeMovementRecovery.cs').read_text(encoding='utf-8-sig')
for rva_text, hex_bytes in re.findall(r'InstallConnectivityObserver\(pendingTransaction, memory, libraryBase, (0x[0-9A-F]+),\s*"([0-9A-F ]+)"', recovery):
    rva = int(rva_text, 16)
...
stub=(root/'Testmods/MoatMove/tests/latest-recovery-stub.bin').read_bytes()
