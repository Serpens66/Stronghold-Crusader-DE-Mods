...
set "PLUGIN_NAME=KeepFlagRotationTest_Serp"
...
pushd "%PROJECT_DIR%"
"%MSBUILD%" tests\KeepFlagRotationTest.Tests.csproj /p:Configuration=Debug
if errorlevel 1 goto build_failed_popd
"%PROJECT_DIR%tests\bin\KeepFlagRotationTest.Tests.exe"
if not "%ERRORLEVEL%"=="0" goto build_failed_popd
popd

if exist "%LOCAL_PLUGIN_DIR%\" rmdir /S /Q "%LOCAL_PLUGIN_DIR%"
pushd "%PROJECT_DIR%"
"%MSBUILD%" KeepFlagRotationTest.csproj /p:Configuration=Debug /p:GameDir="%GAME_DIR%" /p:ExtenderDir="%EXTENDER_DIR%"
if errorlevel 1 goto build_failed_popd
popd

copy /Y "%PROJECT_DIR%info.json" "%LOCAL_PLUGIN_DIR%\info.json" >nul
if not exist "%LOCAL_PLUGIN_DIR%\KeepFlagRotationTest.dll" goto package_failed
if not exist "%LOCAL_PLUGIN_DIR%\KeepFlagRotationTest.pdb" goto package_failed
...
echo Keep Flag Rotation Test built and installed successfully.
if "%NO_PAUSE%"=="0" pause
exit /b 0
