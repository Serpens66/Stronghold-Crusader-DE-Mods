...
Für die isolierte Testphase ist `0xF4930` nun ein aktiver Same-PCL-Hook. Der Funktionsprolog wird separat bytegenau validiert; die Zieladresse wird nie aus einer Call-Site abgeleitet. Der Hook liest den von Vanilla veröffentlichten per-Unit-Pfad, leitet die 1-basierte Unit-ID aus dem validierten Unit-Puffer ab und prüft jede Richtung gegen den unveränderlichen Spieler-Bitset. Ein unsicherer Pfad wird über das read-only Direction-Grid neu gesucht, nibbleweise codiert, erneut geprüft und erst dann vollständig veröffentlicht. Ohne Umweg wird Länge null zurückgegeben. Bei Reentranz, unbekanntem Puffer, Snapshotwechsel, Suchfehler oder fehlgeschlagener Nachprüfung bleiben die ursprünglichen Bytes erhalten beziehungsweise werden zurückgerollt. Es existiert weiterhin kein Direction-Grid-Writer.

Diese erste aktive Stufe deckt reguläre per-Unit-Veröffentlichungen des zentralen Builders ab. Die separaten Angriff-/Gebäudekandidatenpfade `0xDBC60`, `0xDA020` und `0x123090` werden noch nicht verändert; ihre bereits in `BugfixesAndQoL` belegten Verträge werden erst nach der Standard-Builder-Laufzeitabnahme übernommen. Wenn `BugfixesAndQoL_Serp` geladen ist, wird der neue `0xF4930`-Detour vollständig unterdrückt, damit nie zwei Besitzer denselben Funktionshook veröffentlichen.
