    ...
    internal sealed class CustomLordUploadWorkflow
    {
        ...
        internal void Handle(CustomLordUploadRequest request, Action<CustomLordUploadRequest> callOriginal)
        {
            ...
            try
            {
                ...
                if (!TryExtendStaging(request.NameMap, request.MapTitle, out string stagingError))
                {
                    Error("Custom Lord upload [" + request.MapTitle + "] was cancelled: " + stagingError);
            ...
            {
                Error("Custom Lord upload [" + request.MapTitle + "] was cancelled because extended staging failed: " + exception);
                request.FailAction?.Invoke();
                return;
            }
            ...
            Action<CustomLordUploadRequest> callOriginal)
        {
            ...
            {
                if (!TryExtendStaging(request.NameMap, request.MapTitle, out string stagingError))
                    throw new InvalidOperationException(stagingError);
                StartVanillaUpload(
                    request,
                    callOriginal,
        ...
        private bool TryExtendStaging(string uploadContentRoot, string mapTitle, out string error)
        {
            ...
            if (summary == null)
            {
                error = "staging completed without a summary";
                return false;
            }
