    ...
    public sealed class BuiltInCustomizeOriginPacket
    {
        ...
        internal static bool IsValid(BuiltInCustomizeOriginPacket packet)
        {
            if (packet == null || packet.ProtocolVersion != CurrentProtocolVersion || packet.MissionId < 0)
                return false;
            return (packet.TrailType >= ExtendedDataLaunchOriginApi.FirstVanillaTrailType &&
                    packet.TrailType <= ExtendedDataLaunchOriginApi.LastVanillaTrailType) ||
                (packet.TrailType >= ExtendedDataLaunchOriginApi.FirstSandsOfTimeTrailType &&
                 packet.TrailType <= ExtendedDataLaunchOriginApi.LastSandsOfTimeTrailType);
        }
    }
