...
pushd "%PROJECT_DIR%"
"%MSBUILD%" UnitCosts.csproj /p:Configuration=Debug /p:GameDir="%GAME_DIR%" /p:ExtenderDir="%EXTENDER_DIR%"
...
if "%BUILD_EXIT_CODE%"=="0" (
  ...
  copy /Y "%PROJECT_DIR%BepInEx\plugins\UnitCosts\UnitCosts.pdb" "%GAME_DIR%\BepInEx\plugins\UnitCosts\UnitCosts.pdb"
  if errorlevel 1 goto copy_failed
  copy /Y "%PROJECT_DIR%BepInEx\plugins\UnitCosts\info.json" "%GAME_DIR%\BepInEx\plugins\UnitCosts\info.json"
  if errorlevel 1 goto copy_failed
