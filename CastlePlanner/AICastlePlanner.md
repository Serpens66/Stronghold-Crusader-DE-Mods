...
parts of that mechanism may be reusable by `CastlePlanner`.

`CastlePlanner` version `0.2.0` now implements this reuse directly. The former
...
CastlePlanner instead combines `Director.MultiplayerGame` with lobby members not
...
The current `CastlePlanner` code should not return to `JsonUtility`, because it
...
Any additional AIV import for `CastlePlanner` must occur after
...
Since version `0.2.4`, `CastlePlanner` reads the real candidate table directly
...
built, whereas the original CastlePlanner prototype ran them from
...
This is the main advantage over `CastlePlanner`'s former manual
...
## Reuse options for `CastlePlanner`
...
CastlePlanner fixes this at the existing Script Extender detour around
...
Non-Hovel calls and calls outside CastlePlanner's execution window are unchanged.
...
the underlying native pipeline is reusable and is now used by `CastlePlanner`.
