...
- Eingabe, Auswahl, Darstellung und einige native Zustände besitzen auch in Script Extender 2.6.0 kein vollständiges Ereignis. Diese Poller beziehungsweise nativen Hotpath-Hooks dürfen nicht entfernt oder willkürlich gedrosselt werden.
...
- Die Large-Move-Zielmarker besitzen keinen laufenden Gruppen-, Einheiten-, Pfad-, Idle- oder Abschlusszustand mehr. Vanilla behält seine ersten 249 nutzbaren Overlay-Records; ausschließlich danach abgewiesene grüne Bewegungsmarker gelangen für den aktuellen Renderdurchlauf in wiederverwendbare Overflow-Puffer. Vanillas eigener Draw-List-Reset verwirft sie unmittelbar, und der Visible-Tile-Hook ist außerhalb von Overflow-Aktivitätsfenstern und Formation-Previews deaktiviert.
...
- Simulationsticks werden vorerst nicht pauschal gebrokert: Script Extender 2.6.0 stellt mit `OnTick` bereits eine gemeinsame Quelle bereit, und fachliche Tickreihenfolge bleibt im jeweiligen Mod kontrollierbar.
...
- **Kritische Fast-Paths:** Friendly-Moat prüft neue Queries vor dem `Stopwatch`-Zugriff. Large-Move liest im bestehenden Draw-Detour nur während des selektierten Tribe-Passes den nativen Listenzähler und übernimmt ausschließlich grüne Marker, die Vanillas volle Liste abweist. Zwei feste Tile-Head-/Record-/Touched-Tile-Puffer ersetzen Dictionary-Snapshots und Reconciliation. Der Visible-Tile-Hook wird beim ersten Overflow oder Preview aktiviert und nach einem vollständigen nachfolgenden Renderdurchlauf ohne Overflow deaktiviert; Visible-Tile- und Reset-Normalpfad besitzen weder Allokation noch Logging. Der neue Reset-Detour ruft Vanilla immer genau einmal auf.
...
Die installierte `CrusaderDE.dll` und `_inspect/CrusaderDE-Native-Baseline/CURRENT.json` stimmen mit SHA-256 `FBCB93195FC7EFCA9BDAC5204852EFDD76F9818F59A6711750D77C9CEF2831E2` überein. Die semantische Wissensschicht gehört zu Script Extender 2.6.0 und Commit `2cee24e33b5a5d81d1c275efabc714ac59917b7b`.
...
| BugfixesAndQoL: Assassin-Climb-Button | `AssassinClimbRuntime.cs`, dauerhaft nach Initialisierung | Auswahl genau eines steuerbaren Assassinen, UI-Neuerzeugung, Sichtbarkeit | **notwendig** | Script Extender 2.6.0 meldet keine vollständige Auswahländerung. Framepolling mit bestehendem Signaturcache behalten, aber nur in laufender erlaubter Map und bei aktivem Feature arbeiten. Post-UI-Broker ist möglich; Drosselung ist nicht funktionsgleich. |
...
| BugfixesAndQoL: Extended Shift Command Queue | Commandtiming, Completion, Erwartungssignale, Cohort-Koaleszierung; erstellt bei Arbeit temporäre Dictionaries/Listen | **leichter ausführbar** | Ticksemantik der Queue behalten. Large-Move ist aus `OnTick` vollständig entfernt; sein Overflow lebt ausschließlich vom selektierten Vanilla-Overlay-Pass bis zu Vanillas Reset im selben Renderdurchlauf. Nach `currentTick` früh zurückkehren, wenn Queues, Signale und Cohorts leer sind. Arbeitsdictionary/-listen wiederverwenden. Keine Drosselung der eigentlichen Queue, weil sich Ausführungstick und MP-Chore-Reihenfolge ändern würden. |
...
|---|---|---|---|
| Sichtbare-Tile-Overlay: `LargeMoveTargetMarkerRenderer`, Extended-Shift Draw-/Overlaypfade | pro sichtbarem Tile beziehungsweise pro Tribe-/Draw-Aufruf, klar öfter als pro Frame | **notwendig, sichtbarer Tile-Hook im Leerlauf eliminiert** | Vanilla rendert seine ersten 249 Records unverändert. Nur grüne Marker, deren Aufruf bei Zählerstand 250 eintrifft, werden in zwei vorallozierten, tileverketteten Overflow-Puffern gespiegelt und mit den originalen Kategorie-, Sprite-, Layer-, Offset- und Flagwerten über den Vanilla-Sprite-Builder gerendert. `0x41D10` leert nach `0x41D60` den veröffentlichten Overflow unmittelbar; ein nachfolgender Durchlauf ohne Overflow deaktiviert den Tile-Hook. Vorschau und Overflow sind exklusiv. Normalpfade allozieren und protokollieren nicht; es gibt weder Einheitenabfrage noch passübergreifende Reconciliation. |
...
4. In dieser Runde umgesetzt und anschließend korrigiert: Visible-Tile-Hook bedarfsgebunden aktivieren und Large-Move ohne Tickscan oder Reconciliation als Vanilla-Reset-gebundenen Overflow ausführen; Friendly-Moat-Diagnosepfade mit billigeren Inaktivitätsguards beginnen.
