...
- Die Large-Move-Zielmarker benötigen keinen laufenden Einheiten-, Pfad- oder Idle-Scan mehr: Vanillas Markeranforderungen werden vor ihrem 250er-Ausgabelimit pro Overlay-Durchlauf gesammelt und ereignisgebunden abgeglichen. Der Visible-Tile-Hook ist im markerlosen Zustand vollständig deaktiviert.
...
- **Kritische Fast-Paths:** Friendly-Moat prüft neue Queries vor dem `Stopwatch`-Zugriff. Large-Move sammelt Vanillas Markeranforderungen im bestehenden Tribe-Overlay-Detour, gleicht nur am Passende geänderte Tilemengen ab und publiziert nur dann einen neuen Snapshot. Der Visible-Tile-Hook wird beim ersten Vorschau- oder Bewegungsmarker aktiviert und nach dem letzten Marker deaktiviert; sein regulärer Callbackpfad besitzt weder Allokation noch Logging. Native Adressen, Hookgrenzen und Maschinenlogik wurden nicht geändert.
...
|---|---|---|---|
| Sichtbare-Tile-Overlay: `LargeMoveTargetMarkerRenderer`, Extended-Shift Draw-/Overlaypfade | pro sichtbarem Tile beziehungsweise pro Tribe-/Draw-Aufruf, klar öfter als pro Frame | **notwendig, im Leerlauf eliminiert** | Darstellung bleibt im exakten Vanilla-Drawlauf. Der Hook ist ohne Vorschau- oder Bewegungsmarker deaktiviert. Bei aktiven Markern prüft der Callback zuerst den volatilen Aktivzustand und arbeitet danach ausschließlich auf dem unveränderlichen Dictionary-Snapshot; im regulären Pfad ohne Logs oder Allokationen. Der Snapshot entsteht nur bei tatsächlicher Mengenänderung außerhalb des Rendercallbacks. Vanillas vor dem 250er-Limit beobachtete Markeranforderungen sind die autoritative Quelle; drei leere Overlay-Pässe verhindern einen Abschluss durch einen einzelnen transient leeren Durchlauf. |
...
4. In dieser Runde umgesetzt: Visible-Tile-Hook bedarfsgebunden aktivieren, Large-Move-Tickscan durch Vanilla-Overlay-Reconciliation ersetzen und Friendly-Moat-Diagnosepfade mit billigeren Inaktivitätsguards beginnen.
