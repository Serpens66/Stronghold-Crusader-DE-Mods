// Feature: Open customizable setup screens from cooperative trail missions.
...
namespace ExtraFeatures
{
    internal sealed class CoopTrailCustomizeHook : IDisposable
    {
        ...
        private const string CustomizeCoopTrailCommand = "ExtraFeaturesCustomizeCoopTrail";
        ...
        public CoopTrailCustomizeHook(ManualLogSource log)
        {
            ...
            Shared.DebugLogHelper.LogDebug(log, "Extra Features coop trail customize hook installed.");
        }
        ...
        public void Dispose()
        {
            ...
            Shared.DebugLogHelper.LogDebug(log, "Extra Features coop trail customize hook disposed.");
        }
        ...
        private void ButtonClickedHook(FRONT_Multiplayer self, string param)
        {
            ...
            {
                Shared.DebugLogHelper.LogError(log, $"Extra Features failed to open coop trail customize lobby: {ex}");
            }
        }
        ...
        private void CustomizeCurrentCoopTrail(FRONT_Multiplayer self)
        {
            ...
            if (currentMission <= 0)
            {
                Shared.DebugLogHelper.LogWarning(log, $"Extra Features ignored coop trail customize command: unsupportedTrail={currentTrail}, mission={currentMission}.");
                return;
            }

            if (self.currentLobby == null || (!MainViewModel.Instance.Show_CoopHostJoinedPane && !MainViewModel.Instance.Show_CoopClientPane))
            {
                Shared.DebugLogHelper.LogWarning(log, $"Extra Features ignored coop trail customize command outside joined coop lobby: trail={currentTrail}, mission={currentMission}.");
                return;
            }
            ...
            if (self.singlePlayerCoop)
            {
                ...
                Shared.DebugLogHelper.LogDebug(log, () => $"Extra Features switched AI coop trail to singleplayer skirmish setup: trail={currentTrail}, mission={currentMission}.");
                return;
            }

            SwitchHumanCoopToMultiplayerSetup(self, currentTrail);
            Shared.DebugLogHelper.LogDebug(log, () => $"Extra Features switched coop trail to multiplayer setup screen: trail={currentTrail}, mission={currentMission}, isHost={self.currentLobby.isHost}.");
        }
