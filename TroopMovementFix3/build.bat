...
if exist "%LOCAL_SCRIPT_EXTENDER_ROOT%\" (
  if exist "%LOCAL_SCRIPT_EXTENDER_BUILD_OUTPUT%\SHCDESE.dll" (
    ...
    echo Baue zuerst den Script Extender oder entferne den Nebenordner,
...
pushd "%PROJECT_DIR%"
"%MSBUILD%" TroopMovementFix3.csproj /p:Configuration=Release /p:GameDir="%GAME_DIR%" /p:ExtenderDir="%EXTENDER_DIR%"
...
set "PLUGIN_NAME=TroopMovementFix3_Serp"
...
if exist "%GAME_DIR%\BepInEx\plugins\TroopMovementFix_Serp\" (
  echo.
  echo WARNUNG: TroopMovementFix_Serp ist ebenfalls installiert.
  echo Fix3 wird wegen der deklarierten Inkompatibilitaet nicht gleichzeitig geladen.
)

if exist "%GAME_DIR%\BepInEx\plugins\TroopMovementFix2_Serp\" (
  echo.
  echo WARNUNG: TroopMovementFix2_Serp ist ebenfalls installiert.
  echo Fix3 wird wegen der deklarierten Inkompatibilitaet nicht gleichzeitig geladen.
)
