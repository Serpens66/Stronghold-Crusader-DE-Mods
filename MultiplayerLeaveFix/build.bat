...
pushd "%PROJECT_DIR%"
"%MSBUILD%" MultiplayerLeaveFix.csproj /p:Configuration=Debug /p:GameDir="%GAME_DIR%" /p:ExtenderDir="%EXTENDER_DIR%"
...
if "%BUILD_EXIT_CODE%"=="0" (
  ...
  set "PLUGIN_NAME=MultiplayerLeaveFix_Serp"
  set "OLD_PLUGIN_NAME=MultiplayerLeaveFix"
  ...
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%PROJECT_DIR%..\Shared\Release\Write-LocalBuildManifest.ps1" -ModName MultiplayerLeaveFix
  if errorlevel 1 goto copy_failed
  echo Plugin kopiert.
) else (
