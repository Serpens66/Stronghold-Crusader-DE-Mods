...
- `CreateUnitLocal` und `CreateUnitWorld` benennen ihre ersten beiden Parameter in SHCDE-SE derzeit umgekehrt zur nativen Funktion. Für die installierte `CrusaderDE.dll` mit SHA-256 `33AA33457F7DFAAA6D316D1D5E4C5AB97094F2C73B68D349990ABF9D0EF3B469` schreibt die Spawnfunktion bei RVA `0x17FEA0` ihr erstes Argument nach `GameUnit+0x92` (`r_ControllableForPlayerId`) und ihr zweites nach `GameUnit+0x0C` (`r_SpritePlayerColorId`). RandomEvents 1.0.34 übergibt deshalb den reservierten Owner zuerst und Nature-Farbe `0` danach und validiert beide Felder unmittelbar nach dem Spawn.
