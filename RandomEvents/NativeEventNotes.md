...
- `CreateUnitLocal` und `CreateUnitWorld` benennen ihre ersten beiden Parameter in SHCDE-SE derzeit umgekehrt zur nativen Funktion. Für die installierte `CrusaderDE.dll` mit SHA-256 `1E6D4C2E10CC35A7B8082A7E2BCD8BB20680EBEDA803D9B943257B948145CB2B` schreibt die Spawnfunktion bei RVA `0x17FE50` ihr erstes Argument nach `GameUnit+0x92` (`r_ControllableForPlayerId`) und ihr zweites nach `GameUnit+0x0C` (`r_SpritePlayerColorId`). RandomEvents 1.0.34 übergibt deshalb den reservierten Owner zuerst und Nature-Farbe `0` danach und validiert beide Felder unmittelbar nach dem Spawn.
...
- Die Streitkolbenkämpfer werden 0,5 Sekunden nach dem Spawn über den persistenten Tick in Tribes bis fünf Einheiten eingeteilt, auf `Aggressive` gesetzt und mit `IssueMoveHereCommand` zu einem freien, über dieselbe Pfadkomponente erreichbaren Randtile eines zufälligen lebenden Zielspieler-Gebäudes geschickt. Arrayindizes werden dabei explizit in die einbasierten Script-Extender-Gebäude-IDs umgerechnet.
