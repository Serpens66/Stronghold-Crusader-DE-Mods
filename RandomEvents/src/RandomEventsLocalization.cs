...
namespace RandomEvents
{
    internal static class RandomEventsLocalization
    {
        ...
        private static readonly Dictionary<string, string> Fallbacks = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "RandomEvents.ChanceHelp", "Each slider is the independent percentage chance that this event triggers whenever an interval completes." },
            { "RandomEvents.StrengthTitle", "EVENT STRENGTH" },
            { "RandomEvents.ScaledStrengthHelp", "Bandits and archers: X is the unit count per 3 elapsed game months. Units = floor(elapsed months × X ÷ 3). A factor between the selected minimum and maximum is rolled when the event triggers." },
