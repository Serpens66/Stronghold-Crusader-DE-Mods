...
set "PLUGIN_NAME=FearFactorNeutralizationTest_Serp"
...
pushd "%PROJECT_DIR%"
"%MSBUILD%" FearFactorNeutralizationTest.csproj /p:Configuration=Debug /p:GameDir="%GAME_DIR%" /p:ExtenderDir="%GAME_SCRIPT_EXTENDER_DIR%"
if errorlevel 1 goto build_failed_popd
popd

copy /Y "%PROJECT_DIR%info.json" "%LOCAL_PLUGIN_DIR%\info.json" >nul
if not exist "%LOCAL_PLUGIN_DIR%\FearFactorNeutralizationTest.dll" goto package_failed
...
echo Fear Factor Neutralization Test built and installed successfully.
if "%NO_PAUSE%"=="0" pause
exit /b 0
