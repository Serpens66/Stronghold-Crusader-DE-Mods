...
namespace BugfixesAndQoL
{
    ...
    internal sealed unsafe partial class FriendlyMoatMovementRuntime
    {
        ...
        private object log;
        private readonly BugfixesAndQoLViewModel settings = TestSettings.Settings;
        ...
        private void Tests()
        {
            ...
            try
            {
                ...
                *(byte**)(manager+PathManagerOutputBufferOffset) = nativeUnitManager + NativeUnitPathBufferOffset + 2*1000;
                ...
                captureWeighted=true;TestSettings.Settings.RouteMode=0;
                TestUnitMovePipeline(manager, units);
                TestSettings.Settings.RouteMode=1;
                TestUnitMovePipeline(manager, units);
                captureWeighted=false;TestSettings.Settings.RouteMode=0;
        ...
        private void TestUnitMovePipeline(byte* manager, GameUnit* units)
        {
            ...
            {
                NewCommand();
                if(TestSettings.Settings.RouteMode==1)activeMoveCommand.ActiveUnitIdsAtDispatch=Enumerable.Range(1,count).ToArray();
            ...
            *moatPathMode=EnableCompletedMoatModeForScopedMovement((IntPtr)nativeUnitManager,1);
            SetBuilder(1,17);
            if (TestSettings.Settings.RouteMode == 0)
            {
...
namespace Shared {
 ...
 internal static class DebugLogHelper { public static void LogInfo(object log,string text) {} public static void LogWarning(object log,string text) {} public static void LogDebug(object log,string text) {} } internal static class GameModeHelper { public static bool IsMapEditor()=>true; } }
...
namespace BugfixesAndQoL {
 ...
 internal readonly struct MovementOptionsSnapshot {
  ...
  internal static MovementOptionsSnapshot Capture() => Capture(TestSettings.Settings);
  internal static MovementOptionsSnapshot Capture(BugfixesAndQoLViewModel settings) => new MovementOptionsSnapshot(settings.EnableMod,
   settings.RouteMode==1?RouteCalculationMode.RequiredOnly:RouteCalculationMode.Exact);
 }
 internal static class TestSettings {
  internal static readonly BugfixesAndQoLViewModel Settings = new BugfixesAndQoLViewModel();
 }
