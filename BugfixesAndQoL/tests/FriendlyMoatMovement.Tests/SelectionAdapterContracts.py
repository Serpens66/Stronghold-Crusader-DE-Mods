...
rvas = [int(x, 16) for x in re.search(r'SelectionCallRvas =\s*\{([^}]+)', source)[1].split(',')]
...
for rva, span in zip(rvas, spans):
    ...
    emitted = list(dis.disasm((root / f'_inspect/BugfixesAndQoLNativeTests/selection-{rva:X}.bin').read_bytes(), stub))
