...
namespace BugfixesAndQoL
{
    internal sealed class EnemyProximityBulldozeCursorHook : IDisposable
    {
        ...
        // Verified for CrusaderDE.dll SHA-256 FBCB9319... only. The caller disables
        ...
        public EnemyProximityBulldozeCursorHook(ManualLogSource log, BugfixesAndQoLViewModel settings)
        {
            ...
            Shared.DebugLogHelper.LogDebug(log, "Bugfixes and QoL enemy-proximity bulldoze cursor hook installed.");
        }
        ...
        public void Dispose()
        {
            ...
            hook.Undo();
            hook.Dispose();
            Shared.DebugLogHelper.LogDebug(log, "Bugfixes and QoL enemy-proximity bulldoze cursor hook disposed.");
        }
            ...
            if (cursorType == DeleteCursor && ShouldOverrideDeleteCursor(self))
            {
                ...
                catch (Exception ex)
                {
                    if (!queryFailureLogged)
                    {
                        ...
                        Shared.DebugLogHelper.LogError(log, $"Bugfixes and QoL enemy-proximity bulldoze cursor query failed: {ex}");
                    }
                }
            }
        ...
        {
            if (!settings.EnableClientFeatures || !settings.EnableEnemyProximityBulldozeCursorFix ||
                self == null || GetWaitCursorSet(self))
                return false;
