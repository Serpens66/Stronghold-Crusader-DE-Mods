...
pushd "%PROJECT_DIR%"
"%MSBUILD%" AIEconomyProtection.csproj /p:Configuration=Debug /p:GameDir="%GAME_DIR%" /p:ExtenderDir="%EXTENDER_DIR%"
...
if "%BUILD_EXIT_CODE%"=="0" (
  ...
  set "PLUGIN_NAME=AIEconomyProtection_Serp"
  set "OLD_PLUGIN_NAME=AINeverPauses_Serp"
  set "LOCAL_PLUGIN_DIR=%PROJECT_DIR%BepInEx\plugins\!PLUGIN_NAME!"
  set "OLD_LOCAL_PLUGIN_DIR=%PROJECT_DIR%BepInEx\plugins\!OLD_PLUGIN_NAME!"
  ...
  if exist "!OLD_LOCAL_PLUGIN_DIR!\" (
    rmdir /S /Q "!OLD_LOCAL_PLUGIN_DIR!"
    if errorlevel 1 goto copy_failed
  )
