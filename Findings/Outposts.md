...
## Eigene menschliche Rallypoints: Implementierungsstand 17.09.2026

OutpostTest trennt die menschliche Produktion jetzt vom KI-Sammelgruppenpfad. Jeder neue menschliche Maceman erhält eine Ein-Unit-Tribe über GameTribeManagerAPI.Create(owner,false), reguläre Zuweisung, Outpost-Nachinitialisierung und SetStance(Hold). Das vermeidet, dass der nächste Gruppenbefehl ältere Units erneut bewegt. Ein separater menschlicher Produktionszähler zählt erfolgreiche Spawns; die KI behält ihre native Mitgliederzählung/Übergabe. Bestehende menschliche Produktionsgruppen werden einmal abgeschlossen, ohne vorhandene Units umzusetzen.

Die öffentliche Create-API wurde zusätzlich gegen die kanonische DLL aufgelöst: ihr relativer Call bei RVA 0x11E17C führt nach 0x119C00. Dieser Allokator sucht freie Tribe-Slots 1..4499 und initialisiert AliveState 2 sowie Besitzer/Global-ID; sein false-Parameter verändert die aktuelle Gruppenauswahl nicht. Ein Slotmangel liefert 0. Das ist ein anderer Suchpfad als Outposts dedizierter Allokator 0x119D60. Der Native-Hash bleibt FBCB93195FC7EFCA9BDAC5204852EFDD76F9818F59A6711750D77C9CEF2831E2; Baseline-Signaturen bleiben candidate, diese Aussage bezieht sich auf den konkreten geprüften Kontrollfluss.

Rallyauftrag: Ziel beim Spawn kopieren; nach NeedsInit und erfolgreicher Identitäts-/Besitzer-/Mitgliederprüfung einmal den sechsstellig auditierten Win64-Wrapper 0x196100(manager,tribeId,x,y,patrol,flags) mit patrol=0, flags=0x81 aufrufen. Er setzt den Lauf-/Formationstempo-Marker +0x56C, entfernt Bit 7 und gibt newOrder=1 und moveType=1 an 0x11B520. Kein zweiter Detour: der Funktionsentry bleibt im Aufrufweg, insbesondere vorhandene MoatMove-Verarbeitung. Der Wrapper ist void; sein Aufruf allein beweist keine Ankunft. Das nachgelagerte MoveHere-Postevent wird, wenn synchron vorhanden, als Ergebnis geloggt. Einträge werden vor Aufruf aus der Warteschlange entfernt.

Neue fremde MoveHere-Aufträge mit IsNewOrder, Zielbefehle und neue Gruppenzuordnungen können wartende Rallyaufträge abbrechen; zusätzlich werden Unit-/Tribe-Global-ID, Owner, Hold, Gesundheit und genau ein Mitglied vor Ausführung geprüft. Keine rückwirkende Zieländerung. Wie früher gilt: AliveState 2 allein ist kein Gesundheitsnachweis.

Eingabe über InputR3EventHooks.OnKeyDown/Mouse2, Gebäudeauswahl über GetSelectedBuildingId, menschlicher Besitzer über Player-API. KeyManager überwacht Enum.GetValues(KeyCode), einschließlich Mouse2. Die Projektion nutzt die eingefangene Bildschirmposition und GameMap.CalcMapTileFromMousePos; dessen Hover-Nebeneffekte lastMouseLandscapeHeight/overTopHalf werden wiederhergestellt. Darstellung ist eine eigene nicht anklickbare Mesh-Fahne mit Mast, nur bei Auswahl des passenden eigenen Outposts. Keine nativen Spieler-/Rekrutierungskategorien werden als Rallyspeicher verwendet.

ModSaveDataAPI trägt ein versioniertes Binärformat, keine JSON-Abhängigkeit. Gebäude-ID/Global-ID/Besitzer/Typ sichern Zuordnung; Unit-/Tribe-Global-IDs sichern wartende Aufträge. Auch leere Daten werden geschrieben. Die praktische Darstellung, Save/Load während eines noch wartenden Befehls und MoatMove-Integration benötigen Live-Abnahme; Details in [TESTING.md](../Testmods/OutpostTest/TESTING.md). Diese Implementierung ist keine Behauptung bereits bestandener Spieltests.

Eingabe-Nachkontrolle: Der installierte Extender setzt beim Blockieren eines Down-Ereignisses den KeyManager-Zustand auf 0. Bei weiter gehaltener Taste kann daraus erneut Down entstehen. OutpostTest verlangt deshalb im Mouse2-Ereignis zusätzlich die physische Unity-Klickflanke (GetMouseButtonDown(2)). Wartende Klicks werden sowohl zur pausierten Darstellung als auch ausdrücklich vor der nächsten Tick-Produktion übernommen.

## Korrektur: reguläre Auswahl und langlebige Fahne (17.09.2026)

Die erste Rallyfassung übersah zwei bereits dokumentierte Verträge: GetSelectedBuildingId liest die normale Gebäudeauswahl, nicht die Outpost-Inspektionsreferenz; Plugin-LateUpdate endet mit dem Startup-Cleanup. Zusätzlich lehnt Show_HUD_Main als alleinige Bedingung ein regulär geöffnetes Gebäude-HUD ab. Der bisherige Lifecycle-Präflight prüfte Teardown, aber keine dauerhaften Plugin-Update-Methoden. Er wurde entsprechend erweitert. Erfolgreiche synthetische Tests der Produktion belegten diese UI-Integration nicht.

Die zweite Outpost-Editorprüfung bei RVA 0x8A1F9 wird nun optional freigegeben; die vorgelagerten Besitzer-/Restriktionsprüfungen einschließlich 0x8A051 bleiben unverändert. Native Referenzhash weiterhin FBCB93195FC7EFCA9BDAC5204852EFDD76F9818F59A6711750D77C9CEF2831E2. Typen 2/106/107 verwenden Panel 45. Pending-ID -> Auswahlübernahme -> HUD-Export -> GameData/InBuildingGameAction bleibt Vanilla. Panel 45 exportiert Profilmaske/Größe/Verzögerung unabhängig vom Editor; beim Aufbau verhindert inSetup Slider-Schreibbefehle. Manuelle Buttons/Slider sind in diesem Test auch im Spiel bedienbar. Keine Prozent- oder Read-only-UI implementiert.

Mittelklick und Projektion laufen unmittelbar im Tastenevent auf dem Unity-Thread. Render liest nur die aktuelle gültige Auswahl und per Dictionary ein Ziel, ohne Gebäudescans oder Simulation. Kein Warten auf die Produktionssperre; keine Sperre während Unity-Aufrufen. Ohne Ziel keine Kartenprojektion; mit Ziel genau ein Tile, wiederverwendete Fahne und nur geänderte Transform-/Sichtbarkeitswerte. Darstellung wurzelt in Application.onBeforeRender statt im Startup-Plugin. Fehler deaktivieren den betroffenen Eingabe-/Darstellungspfad, nicht die unabhängige Produktion.

Für den nächsten Live-Test gibt es einmalig ein Messfenster: 60 aktive Frames Vorlauf, anschließend 1800 Frames, je Kategorie 30 Aufwärmbeobachtungen und maximal 300 Stichproben. Kategorien: 0 keine gültige Outpost-Auswahl, 1 Outpost ohne Ziel, 2 Outpost mit Ziel. Ausgabe enthält Mittel-/Maximaldauer in Mikrosekunden und Thread-Allokationsbytes, falls die Mono-API existiert. Fehlende Kategorien haben samples=0; unbekannte Allokationen werden nicht als null behauptet. Keine permanente Frame-Protokollierung. Eigene Ingame-Leistungswerte liegen noch nicht vor.