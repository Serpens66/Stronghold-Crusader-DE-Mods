...
| `0x199C00` | erzeugt einen Tribe-Gebäudeangriff mit Modus 9 und Building-ID/UID |

## Zustandsfluss

1. State 4 prüft über `0xCF360`, ob ein Zugang zum Ziel besteht. Bei Erfolg folgt State 6, andernfalls State 5.
2. State 5 bearbeitet Befestigungen und prüft erneut über `0xCF360` und `0xCF400`. Ein gefundener Zugang führt zu State 6.
3. State 6 ruft `0xCF020` und danach `0x3B450` auf.
...
5. State 6 kann die Zuteilung wiederholt ausführen und bestehende Befehle erneut setzen.

Ein fehlgeschlagener Lord-Bewegungsauftrag lässt `0x3B450` mit Fehler zurückkehren. Erst wenn anschließend beide Erreichbarkeitsprüfungen scheitern, fällt die FSM nach State 5 zurück. Rückzug und Auflösung erfolgen über getrennte spätere Zustände.

## Ursache in `RVA 0x3B450`

Die Routine verarbeitet vollständige AI-Tribes, keine einzelnen Soldaten. Die statischen Paare sind Tribe-Rollen mit maximaler Slotanzahl:

```text
(192,8), (190,2), (12,1), (11,1), (186,3), (189,1),
(15,2), (17,1), (18,1), (14,1), (13,1)
```

Bestätigte Rollen sind unter anderem `192 = SiegeWallTribe`, `190 = SiegeReserveTribe` und `186 = SiegeCoverTribe`. Die Rollen 13, 14 und 18 werden in diesem Verteiler übersprungen; Rolle 186 darf nicht in den Lord-Zweig.
...
- Ein Treffer erzeugt über `0x199C00` einen Gebäudeangriff für den gesamten Tribe.

Die Auswahl verwendet drei Tabellen mit jeweils 47 Gebäudetypen. Für die Beobachtung relevant:

- Typ `3`: Holzfällerhütte.
- Typ `32`: Apfelfarm.
...
Findet `0x30E90` kein Gebäude, wählt `0x3B450` einen von `0xCF020` vorberechneten Angriffsvektoren. Rolle 186 beginnt vier Einträge später. Liegt der Tribe nicht bereits nahe genug am Punkt, bewegt `0x11B520` ihn dorthin. Dieser Fallback wählt den Lord nicht erneut und passt zum beobachteten Sammelpunktverhalten.
...
- Die Leser laufen in State 5 für die Rollen 11, 12, `SiegeWallTribe` und `SiegeReserveTribe`.
- Der Gruppenmodus wechselt von `0x3F2` auf `0x420`.
...
`Improved Siege` beeinflusst damit die Befestigungsbearbeitung in State 5. Es ändert weder die Lord-Quote in `0x3B450` noch die Gebäudeauswahl in `0x30E90`.
...
- Die wiederholte State-6-Ausführung kann Aufträge erneut setzen. Eine ausdrücklich durch Verluste ausgelöste Nachbesetzung ist nicht belegt.
...
- Bit 0 wirkt in den geprüften State-5-Pfaden wie die globale Option `Improved Siege`.
...
Die Juni-Vergleichs-DLL und die aktuelle DLL besitzen für die zentralen Routinen denselben normalisierten Kontrollfluss:

| Juni-RVA | Aktuelle RVA | Funktion |
| --- | --- | --- |
| `0x30E30` | `0x30E90` | Gebäudezuteiler |
| `0x3A840` | `0x3B450` | Breach-/Lord-Verteiler |
| `0x3B6D0` | `0x3C2E0` | Angriffs-FSM |
...
- Ein gezielter Laufzeittrace sollte pro State-6-Aufruf Tribe-ID, Rolle, Slot, Lord-Zweig, Gebäudeziel und Fallbackkoordinate protokollieren.
